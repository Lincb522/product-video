...
- **画面**：单张截图、两张并排对照、章节切换、截图间淡入淡出。截图保持比例，不裁掉边缘。
- **模拟操作**：按配置移动鼠标、展示点击位置，再切换到操作后的截图。视频内标注“操作演示”。这是成片里的鼠标动画；前面的自动采集阶段会实际执行采集计划中的界面操作。
...
./run.sh preview-voice --voice '小何 2.0' --text '你可以把文稿和产品截图放进项目，生成自己的介绍视频。'
...
| `steps[].at` | 画面切换点，占本章**配音时长**的比例；第一项 0，后续递增且小于 1 |
| `images` / `labels` | 1–2 张图片及同数量标签；PNG、JPEG 等 Pillow 可读取的图片 |
| `cursor` | 相对原截图的 `[x, y]`，左上角 `[0,0]`、右下角 `[1,1]`，只用于单图 |
| `click` | 鼠标移动后显示点击圆环；应预留至少约 1 秒显示该步骤 |
...
截图使用完整内容等比放入画面；横竖比例不同会留边。过长的标题会缩至最低字号，仍放不下时明确报错，不省略文字。
