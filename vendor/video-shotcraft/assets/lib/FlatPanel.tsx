...
export const FlatPanel: React.FC<{
    ...
    return (<PvElement as={"group"} position={position} rotation={[0, yaw, 0]}>
      {shadow ? (<PvElement as={"mesh"} rotation={[-Math.PI / 2, 0, 0]} position={[0.05, -0.012, 0.06]}>
          <PvElement as={"planeGeometry"} args={[width * 1.3, height * 1.3]}/>
          <PvElement as={"meshBasicMaterial"} map={shadowTex} transparent opacity={0.85 * opacity} depthWrite={false}/>
        </PvElement>) : null}
      <PvElement as={"mesh"} rotation={[-Math.PI / 2, 0, 0]} material={mat}>
        <PvElement as={"planeGeometry"} args={[width, height]}/>
      </PvElement>
    </PvElement>);
};
