...
const LoopLayer: React.FC<{
    ...
    return (<PvElement as={OffthreadVideo} src={staticFile(src)} muted={muted} volume={volume} startFrom={startFrom} style={{
            position: 'absolute',
            inset: 0,
            width: size,
            height: size,
...
export const ClipCard: React.FC<{
...
}> = ({ src, caption, size = 560, radius = 20, muted = true, startFrom = 0, style, captionSize = 17, loopDurationInFrames, loopCrossfadeInFrames = 8, durationInFrames, }) => {
    ...
    const step = loopDurationInFrames
        ...
        : 0;
    const canLoop = !!loopDurationInFrames &&
    ...
    if (canLoop) {
        ...
        video = (<PvElement as={"div"} style={{ position: 'relative', width: size, height: size }}>
        {Array.from(pvArrayCopy({ length: n }), (_, i) => (<Sequence key={i} from={i * step} durationInFrames={loopDurationInFrames} layout="none">
        ...
        video = (<PvElement as={OffthreadVideo} src={staticFile(src)} muted={muted} startFrom={startFrom} style={{
                width: size,
                height: size,
                objectFit: 'cover',
                display: 'block',
    ...
    return (<PvElement as={"div"} style={{
      ...
      {video}
      {caption ? (<PvElement as={"div"} style={{
                height: capH,
                display: 'flex',
                alignItems: 'center',
                padding: '0 18px',
          ...
          <PvElement as={"span"} style={{ color: ACCENT, marginRight: 10 }}>▸</PvElement>
          {caption}
        </PvElement>) : null}
    </PvElement>);
};
