...
export const AvatarGridRadialBuildColorize: React.FC = () => {
    ...
    return (<DesignStage bg={PAPER} raster="zoom">
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ position: 'absolute', inset: 16 }}>
          {CELLS.map((cl, i) => {
            const r = Math.floor(i / COLS);
            const c = i % COLS;
            ...
            return (<PvElement as={"div"} key={i} style={{
                ...
                {cl.kind === 2 && (<PvElement as={"div"} style={{
                        position: 'absolute',
                        inset: 0,
                        borderRadius: 7,
        ...
        <PvElement as={"div"} style={{
          ...
          {LEGEND.map(([txt, col]) => (<PvElement as={"div"} key={txt} style={{
                display: 'flex',
                alignItems: 'center',
                gap: 5,
                fontWeight: 600,
              ...
              <PvElement as={"span"} style={{ width: 6, height: 6, borderRadius: '50%', background: col, display: 'inline-block' }}/>
              <PvElement as={"span"}>{txt}</PvElement>
            </PvElement>))}
        </PvElement>
      </PvElement>
    </DesignStage>);
