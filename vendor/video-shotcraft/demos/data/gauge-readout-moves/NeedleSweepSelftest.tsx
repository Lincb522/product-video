...
const Gauge: React.FC<{
    ...
    for (let k = 0; k <= 30; k++) {
        ...
        ticks.push(<PvElement as={"line"} key={k} x1={x0} y1={y0} x2={x1} y2={y1} stroke={dd >= 225 ? RED : G.mid} strokeWidth={major ? 4 : 2}/>);
    }
    return (<PvElement as={"div"} style={{ width: GA_W, height: 480, position: 'relative' }}>
      <PvElement as={"svg"} width={GA_W} height={430}>
        {/* 弧形轨道 + 红区 */}
        <PvElement as={"path"} d={arcPath(0, 270, R)} fill="none" stroke={G.line} strokeWidth={10} strokeLinecap="round"/>
        <PvElement as={"path"} d={arcPath(225, 270, R)} fill="none" stroke={RED} strokeWidth={10} strokeLinecap="round" opacity={0.85}/>
        {ticks}
        {/* 指针：琥珀刚体，rotate 全量程往返 */}
        <PvElement as={"g"} transform={`rotate(${d.toFixed(3)} ${CX} ${CY})`}>
          <PvElement as={"line"} x1={tailX} y1={tailY} x2={tipX} y2={tipY} stroke={AMBER} strokeWidth={9} strokeLinecap="round"/>
        </PvElement>
        <PvElement as={"circle"} cx={CX} cy={CY} r={15} fill={G.ink}/>
        <PvElement as={"circle"} cx={CX} cy={CY} r={6} fill={AMBER}/>
      </PvElement>
      {/* 落定同帧弹出的数值 */}
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"span"} style={{ fontFamily: 'Helvetica, Arial, sans-serif', fontWeight: 800, fontSize: 62, color: G.ink }}>
          {value}
        </PvElement>
        <PvElement as={"span"} style={{ fontFamily: 'Helvetica, Arial, sans-serif', fontWeight: 700, fontSize: 30, color: G.mid, marginLeft: 8 }}>
          %
        </PvElement>
      </PvElement>
    </PvElement>);
...
export const NeedleSweepSelftest: React.FC = () => {
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ height: 16, width: 320, background: G.bar, borderRadius: 8 }}/>
        <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 96, display: 'flex' }}>
          {GAUGES.map((g, i) => (<Gauge key={i} start={g.start} target={g.target}/>))}
        </PvElement>
      </PvElement>
    </PvElement>);
};
