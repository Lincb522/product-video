...
const SUB = 20;
const Y0 = 390;
const cellPts = (x: number, y: number): [
    number,
    number
][] => [
    [x, y],
...
const buildDigit = (bitmap: string[], x0: number): [
    number,
    number
][] => {
    const out: [
    ...
    bitmap.forEach((rowStr, r) => {
        rowStr.split('').forEach((c, col) => {
            if (c === '1')
                out.push(...cellPts(x0 + col * CELL, Y0 + r * CELL));
        });
    });
    return out;
};
...
export const UnitDotSwarmRegroupV2: React.FC = () => {
    ...
    const dots = Array.from(pvArrayCopy({ length: N }), (_, i) => {
        const stag = rnd(i, 7) * STAG;
        const mig = (start: number) => spring({
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ width: 18, height: 18, borderRadius: 9, background: G.mid }}/>
        Each dot ≈ 40 customers
      </PvElement>

      <PvElement as={"svg"} width={1920} height={1080} style={{ position: 'absolute', inset: 0 }}>
        {dots.map((p, i) => (<PvElement as={"circle"} key={i} cx={p[0]} cy={p[1]} r={DOT_R} fill={groupOf(i) === 1 ? AMBER : groupOf(i) === 2 ? G.ink : G.mid}/>))}
      </PvElement>

      {/* 阶段 1：簇标签（真人数） */}
      {clusterLabelOp > 0 &&
            CLUSTER_C.map((c, g) => (<PvElement as={"div"} key={`cl${g}`} style={{
                    position: 'absolute',
                    left: c[0] - 200,
      ...
      {/* 阶段 2：柱底基线 + 真轴标 */}
      {barLabelOp > 0 && (<>
          <PvElement as={"div"} style={{ position: 'absolute', left: 380, width: 1160, top: BAR_BASE + 16, height: 3, background: G.bar, opacity: barLabelOp }}/>
          {BAR_X.map((x, g) => (<PvElement as={"div"} key={`bl${g}`} style={{
                    position: 'absolute',
                    left: x - 150,
                    top: BAR_BASE + 30,
                    width: 300,
      ...
      {/* 终幕：数字下方真文案 */}
      {captionOp > 0 && (<PvElement as={"div"} style={{
                position: 'absolute',
                left: 0,
                width: 1920,
                top: Y0 + 7 * CELL + 60,
