...
const env = (x: number): number => {
    if (x <= SPIKE_X0 || x >= SPIKE_X0 + SPIKE_W)
        return 1;
...
// 波形：四个正弦叠加，x 单位 = 世界像素
const wave = (x: number): number => 0.34 * Math.sin(x * 0.021) +
...
export const OscilloscopeStreamV2: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ position: 'absolute', top: 110, width: '100%', textAlign: 'center' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        {/* 真卡头：标题 + 副题 + 实时读数 */}
        <PvElement as={"div"} style={{ position: 'absolute', left: PAD, top: 38, fontFamily: 'Helvetica, Arial, sans-serif' }}>
          <PvElement as={"div"} style={{ fontSize: 30, fontWeight: 700, color: G.ink }}>Requests per second</PvElement>
          <PvElement as={"div"} style={{ fontSize: 20, fontWeight: 500, color: G.mid, marginTop: 8 }}>api-gateway · production · last 60 s</PvElement>
        </PvElement>
        <PvElement as={"div"} style={{
            position: 'absolute',
            right: PAD,
          ...
          <PvElement as={"div"} style={{ fontSize: 46, fontWeight: 800, color: hot ? AMBER : G.ink, fontVariantNumeric: 'tabular-nums', letterSpacing: -1 }}>
            {readout}
          </PvElement>
          <PvElement as={"div"} style={{ fontSize: 18, fontWeight: 600, color: hot ? AMBER : G.mid, marginTop: 2 }}>req/s · live</PvElement>
        </PvElement>

        {/* y 轴真刻度 */}
        {Y_TICKS.map((t, i) => (<PvElement as={"div"} key={`yt${i}`} style={{
                position: 'absolute',
                left: PAD - 6,
        ...
        <PvElement as={"div"} style={{ position: 'absolute', left: PLOT_X, top: PLOT_Y, width: PLOT_W, height: PLOT_H }}>
          {/* 网格 */}
          {[0, 1, 2, 3, 4].map((i) => (<PvElement as={"div"} key={`h${i}`} style={{ position: 'absolute', left: 0, right: 0, top: (PLOT_H / 4) * i, height: 2, background: G.line, opacity: 0.7 }}/>))}
          {[0, 1, 2, 3, 4, 5, 6].map((i) => (<PvElement as={"div"} key={`v${i}`} style={{ position: 'absolute', top: 0, bottom: 0, left: (PLOT_W / 6) * i, width: 2, background: G.line, opacity: 0.45 }}/>))}

          <PvElement as={"svg"} width={PLOT_W} height={PLOT_H} style={{ position: 'absolute', inset: 0, overflow: 'visible' }}>
            <PvElement as={"polyline"} points={pts.join(' ')} fill="none" stroke={G.ink} strokeWidth={5} strokeLinejoin="round" strokeLinecap="round"/>
            {!frozen && glowOp > 0 && (<PvElement as={"polyline"} points={tailPts.join(' ')} fill="none" stroke="#6a6a68" strokeWidth={9} strokeLinecap="round" strokeLinejoin="round" opacity={0.55 * glowOp} style={{ filter: 'blur(2px)' }}/>)}
            {/* 写入点亮点（1.5×） */}
            {!frozen && glowOp > 0 && (<>
                <PvElement as={"circle"} cx={PLOT_W} cy={headY} r={24} fill={spikeK > 0.15 ? AMBER : '#8f8f8d'} opacity={0.35 * glowOp} style={{ filter: 'blur(5px)' }}/>
                <PvElement as={"circle"} cx={PLOT_W} cy={headY} r={10.5} fill={dotColor} opacity={glowOp}/>
              </>)}
          </PvElement>
        </PvElement>

        {/* x 轴真时间刻度 */}
        <PvElement as={"div"} style={{ position: 'absolute', left: PLOT_X, top: PLOT_Y + PLOT_H + 14, width: PLOT_W }}>
          {X_TICKS.map((t, i) => (<PvElement as={"div"} key={`xt${i}`} style={{
                position: 'absolute',
                left: (PLOT_W / 6) * i - 40,
                width: 80,
                textAlign: 'center',
