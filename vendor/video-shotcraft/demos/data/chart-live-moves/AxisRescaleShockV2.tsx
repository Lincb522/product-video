...
export const AxisRescaleShockV2: React.FC = () => {
    ...
    let headX = basePts.length ? Number(pvCopy(basePts[basePts.length - 1]).split(',')[0]) : 0;
    let headY = basePts.length ? Number(pvCopy(basePts[basePts.length - 1]).split(',')[1]) : 0;
    let shockSeg: string[] = [];
    if (shockT > 0) {
        const x0 = xOf(N - 2);
    ...
    const kick = frame >= SHOCK_END && frame < SHOCK_END + 8
        ? 8 * (1 - (frame - SHOCK_END) / 8) * Math.sin((frame - SHOCK_END) * 2.6)
        : 0;
    const shockOn = frame >= DRAW_END; // 爆表段样式生效期
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ position: 'absolute', top: 100, width: '100%', textAlign: 'center' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        {/* 真卡头 */}
        <PvElement as={"div"} style={{ position: 'absolute', left: PAD, top: 34, fontFamily: 'Helvetica, Arial, sans-serif' }}>
          <PvElement as={"div"} style={{ fontSize: 30, fontWeight: 700, color: G.ink }}>Monthly revenue</PvElement>
          <PvElement as={"div"} style={{ fontSize: 19, fontWeight: 500, color: G.mid, marginTop: 6 }}>FY2026 · all products · USD</PvElement>
        </PvElement>

        <PvElement as={"div"} style={{ position: 'absolute', left: PLOT_X, top: PLOT_Y, width: PLOT_W, height: PLOT_H }}>
          {/* 基础网格 4 根 + 重标同帧加密出的 4 根 */}
          {[0, 1, 2, 3, 4].map((i) => (<PvElement as={"div"} key={`g${i}`} style={{ position: 'absolute', left: 0, right: 0, top: (PLOT_H / 4) * i, height: 2, background: G.line }}/>))}
          {[1, 3, 5, 7].map((i) => (<PvElement as={"div"} key={`gd${i}`} style={{ position: 'absolute', left: 0, right: 0, top: (PLOT_H / 8) * i, height: 1.5, background: G.line, opacity: 0.8 * denseOp }}/>))}

          {/* 刻度：同一格位，旧值下飞淡出 / 新值上方滑入 */}
          {OLD_TICKS.map((v, i) => {
            const y = (PLOT_H / 4) * (3 - i);
            return (<PvElement as={"div"} key={`t${i}`} style={{ position: 'absolute', left: -AXIS_W, top: y - 13, width: AXIS_W - 14, height: 26, textAlign: 'right' }}>
                <PvElement as={"div"} style={{
                    position: 'absolute',
                    inset: 0,
          ...
          <PvElement as={"div"} style={{ position: 'absolute', left: -AXIS_W, top: PLOT_H - 13, width: AXIS_W - 14, fontFamily: 'Helvetica', fontWeight: 700, fontSize: 21, color: G.mid, textAlign: 'right' }}>$0</PvElement>

          {/* 图表上沿强调线 */}
          <PvElement as={"div"} style={{ position: 'absolute', left: 0, right: 0, top: 0, height: 3, background: G.bar }}/>

          <PvElement as={"svg"} width={PLOT_W} height={PLOT_H} style={{ position: 'absolute', inset: 0, overflow: 'visible' }}>
            {/* 历史段 */}
            <PvElement as={"polyline"} points={basePts.join(' ')} fill="none" stroke={G.ink} strokeWidth={5} strokeLinejoin="round" strokeLinecap="round"/>
            {/* 爆表段：琥珀 + 10px，重标落回后收敛回常规 */}
            {shockSeg.length > 0 && (<PvElement as={"polyline"} points={shockSeg.join(' ')} fill="none" stroke={AMBER} strokeWidth={shockOn && frame < RESCALE_END ? 10 : 6} strokeLinejoin="round" strokeLinecap="round"/>)}
            {/* 端点标记 */}
            {markS > 0 && (<>
                <PvElement as={"circle"} cx={headX} cy={headY} r={13 * markS} fill={AMBER}/>
                <PvElement as={"circle"} cx={headX} cy={headY} r={22 * markS} fill="none" stroke={AMBER} strokeWidth={3} opacity={0.55}/>
              </>)}
          </PvElement>

          {/* 真值标签 "$340k"（端点旁弹出） */}
          {valS > 0 && (<PvElement as={"div"} style={{
                position: 'absolute',
                left: headX - 178,
                top: headY - 27,
                width: 150,
          ...
          {/* x 轴真月份 */}
          {MONTHS.map((m, i) => (<PvElement as={"div"} key={`m${i}`} style={{
                position: 'absolute',
                left: xOf(i) - 30,
                top: PLOT_H + 16,
                width: 60,
