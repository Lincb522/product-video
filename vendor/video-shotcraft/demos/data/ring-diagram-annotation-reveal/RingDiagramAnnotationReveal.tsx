...
const p = (frame: number, start: number, end: number) => interpolate(frame, [start, Math.max(start + 1, end)], [0, 1], {
    ...CLAMP,
...
export const RingDiagramAnnotationReveal: React.FC = () => {
    ...
    const arrows = Array.from(pvArrayCopy({ length: 12 }), (_, i) => {
    ...
    return (<DesignStage bg="#f7f7f5" raster="zoom">
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, overflow: 'hidden', fontFamily: 'Arial, sans-serif' }}>
        <PvElement as={"div"} style={{
            position: 'absolute',
            inset: 0,
            background: '#c7c7c4',
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"svg"} viewBox="0 0 480 270" style={{ position: 'absolute', left: -130, top: -12, width: 480, height: 270 }}>
            <PvElement as={"defs"}>
              <PvElement as={"marker"} id="ring-arrow-head" markerWidth="5" markerHeight="5" refX="4.5" refY="2.5" orient="auto">
                <PvElement as={"path"} d="M0,0 L5,2.5 L0,5 Z" fill="#303032"/>
              </PvElement>
            </PvElement>
            <PvElement as={"g"} stroke="#454548" strokeWidth="0.75" markerEnd="url(#ring-arrow-head)" opacity={arrowsIn}>
              {arrows.map(({ x1, y1, x2, y2 }) => (<PvElement as={"line"} key={`${x1}-${y1}`} x1={x1} y1={y1} x2={interpolate(arrowsIn, [0, 1], [x1, x2])} y2={interpolate(arrowsIn, [0, 1], [y1, y2])}/>))}
            </PvElement>
          </PvElement>
          <PvElement as={"div"} style={{
            position: 'absolute',
        ...
        <PvElement as={"div"} style={{ position: 'absolute', left: 321, top: 93, display: 'flex', gap: 1.5 }}>
          {['#59595c', '#69696c', '#858588', '#a7a7aa'].map((shade, i) => (<TitleBlock key={shade} frame={frame} start={112 + i * 3} shade={shade} index={i}/>))}
        </PvElement>
        <PvElement as={"div"} style={{
            position: 'absolute',
            left: 310,
