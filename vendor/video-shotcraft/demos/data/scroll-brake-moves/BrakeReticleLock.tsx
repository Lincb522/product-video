...
const Row: React.FC<{
    i: number;
    highlight: number;
}> = ({ i, highlight }) => (<PvElement as={"div"} style={{
        position: 'absolute',
        top: i * PITCH,
        left: 0,
        width: LIST_W,
    ...
    <PvElement as={"div"} style={{ height: 14, width: `${28 + ((i * 31) % 34)}%`, background: G.bar, borderRadius: 7 }}/>
    <PvElement as={"div"} style={{ height: 10, width: `${12 + ((i * 17) % 18)}%`, background: G.line, borderRadius: 5 }}/>
...
}> = ({ flip, style }) => (<PvElement as={"div"} style={{ position: 'absolute', width: 46, height: 46, transform: `scale(${flip[0]}, ${flip[1]})`, ...style }}>
    <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, width: 46, height: 8, background: G.ink, borderRadius: 3 }}/>
    <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, width: 8, height: 46, background: G.ink, borderRadius: 3 }}/>
  </PvElement>);
export const BrakeReticleLock: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      {/* 顶栏（不随滚动） */}
      <PvElement as={"div"} style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 72, background: G.panel, borderBottom: `2px solid ${G.line}`, display: 'flex', alignItems: 'center', padding: '0 32px', gap: 20, boxSizing: 'border-box', zIndex: 3 }}>
        <PvElement as={"div"} style={{ height: 18, width: 220, background: G.bar, borderRadius: 9 }}/>
        <PvElement as={"div"} style={{ marginLeft: 'auto', width: 36, height: 36, borderRadius: 18, background: G.mid }}/>
      </PvElement>

      {/* changelog 长列表：整体上掠，速度段 blur */}
      <PvElement as={"div"} style={{ position: 'absolute', left: LIST_X, top: 0, width: LIST_W, height: 1080, filter: blur > 0.5 ? `blur(${blur}px)` : 'none' }}>
        <PvElement as={"div"} style={{ position: 'absolute', top: -scroll, left: 0, width: LIST_W, height: 40 * PITCH }}>
          {Array.from(pvArrayCopy({ length: 38 })).map((_, i) => (<Row key={i} i={i} highlight={i === TARGET_ROW ? highlight : 0}/>))}
        </PvElement>
      </PvElement>

      {/* 准星角标：急刹帧同帧挂载（组合命门：与刹停同帧共振） */}
      {f >= BRAKE &&
            corners.map((c, i) => (<Corner key={i} flip={c.flip} style={{ left: c.x + c.fromX * fly, top: c.y + c.fromY * fly, opacity: Math.min(1, lockT * 3 + 0.35) }}/>))}

      {/* 旁弹小标签 */}
      {f >= BRAKE + 4 && (<PvElement as={"div"} style={{
                position: 'absolute',
