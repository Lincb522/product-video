...
const scrollAt = (f: number): number => interpolate(f, [SCROLL0, SCROLL1], [START_T, END_T], {
...
const Row: React.FC<{
    ...
    return (<PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{ width: 88, height: 26, borderRadius: 13, background: isTarget ? '#4a4a48' : G.mid, flexShrink: 0 }}/>
      <PvElement as={"div"} style={{ height: 16, width: `${titleW}%`, background: G.bar, borderRadius: 8 }}/>
      <PvElement as={"div"} style={{ marginLeft: 'auto', height: 12, width: 110, background: G.line, borderRadius: 6, flexShrink: 0 }}/>
    </PvElement>);
};
export const ChangelogScrollBrake: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      <PvElement as={"div"} style={{
        ...
        {Array.from(pvArrayCopy({ length: N })).map((_, i) => (<Row key={i} i={i} frame={frame}/>))}
      </PvElement>
    </PvElement>);
};
