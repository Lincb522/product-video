...
const TickStop: React.FC<{
    ...
    return (<PvElement as={"div"} style={{ position: 'absolute', left: tick.x, top: 0 }}>
      {/* 刻度竖线 */}
      <PvElement as={"div"} style={{ position: 'absolute', left: -3, top: AXIS_Y - 28, width: 6, height: 56, background: G.ink, borderRadius: 3 }}/>
      {/* 刻度标签 */}
      <PvElement as={"div"} style={{ position: 'absolute', left: -80, top: AXIS_Y + 44, width: 160, textAlign: 'center', fontFamily: 'Helvetica, Arial, sans-serif', fontWeight: 800, fontSize: 40, color: G.ink }}>
        {tick.label}
      </PvElement>
      {/* 卡片从刻度线弹立：以底边为轴 scaleY 0→1（带过冲），伴随上移 */}
      {appeared && (<PvElement as={"div"} style={{
                position: 'absolute',
                left: -CARD_W / 2,
...
export const TimelineTravel: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: W, height: 1080, background: G.bg, overflow: 'hidden', position: 'relative' }}>
      {/* 推近层：以画面中央偏下（末刻度落点）为原点放大 */}
      <PvElement as={"div"} style={{ width: W, height: 1080, transform: `scale(${zoom})`, transformOrigin: '50% 62%' }}>
        {/* 世界层：唯一横移的容器 */}
        <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, width: WORLD_W, height: 1080, transform: `translateX(${-camX}px)` }}>
          {/* 主轴线 */}
          <PvElement as={"div"} style={{ position: 'absolute', left: 200, top: AXIS_Y - 3, width: WORLD_W - 400, height: 6, background: G.bar, borderRadius: 3 }}/>
          {/* 次刻度（小点，增强速度感） */}
          {Array.from(pvArrayCopy({ length: 22 })).map((_, i) => (<PvElement as={"div"} key={i} style={{ position: 'absolute', left: 960 + i * (TICK_GAP / 5) - 2, top: AXIS_Y - 12, width: 4, height: 24, background: G.bar, borderRadius: 2 }}/>))}
          {TICKS.map((_, i) => (<TickStop key={i} i={i} frame={frame}/>))}
        </PvElement>
      </PvElement>
      <PvElement as={"div"} style={{ position: 'absolute', top: 90, width: '100%', textAlign: 'center' }}>
