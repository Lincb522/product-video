...
const Strip: React.FC<{
    ...
    {Array.from(pvArrayCopy({ length: 20 })).map((_, k) => (<PvElement as={"div"} key={k} style={{
                width: DW,
                height: ROW,
                lineHeight: `${ROW}px`,
                textAlign: 'center',
...
const DigitReel: React.FC<{
    ...
    return (<PvElement as={"div"} style={{ position: 'relative', width: DW, height: ROW, overflow: 'hidden' }}>
      {gate > 0.001 && (<>
...
// 不滚动的静态字符（小数点 / %），一直驻场
const StaticGlyph: React.FC<{
    ch: string;
    color: string;
    w?: number;
}> = ({ ch, color, w }) => (<PvElement as={"div"} style={{
        width: w,
        height: ROW,
        lineHeight: `${ROW}px`,
        textAlign: 'center',
...
export const OdometerDigitRoll: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ width: 520, height: 22, background: G.bar, borderRadius: 11 }}/>
        <PvElement as={"div"} style={{ width: 320, height: 14, background: G.line, borderRadius: 7 }}/>
      </PvElement>
    </PvElement>);
};
