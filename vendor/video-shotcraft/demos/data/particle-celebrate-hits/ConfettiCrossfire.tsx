...
const makeGun = (originDeg: number, saltBase: number): Confetto[] => Array.from({ length: 50 }).map((_, i) => {
...
export const ConfettiCrossfire: React.FC = () => {
    ...
    const renderGun = (gun: Confetto[], origin: {
        x: number;
        y: number;
    }, keyBase: string) => gun.map((c, i) => {
        if (age <= 0)
            return null;
        ...
        // 落出画外即卸载（左右也裁）
        if (y > 1140 || x < -80 || x > 2000)
            return null;
        const rot = c.phase + c.spin * age;
        return (<PvElement as={"div"} key={`${keyBase}${i}`} style={{
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ height: 10, width: 150, background: G.line, borderRadius: 5 }}/>
      </PvElement>
