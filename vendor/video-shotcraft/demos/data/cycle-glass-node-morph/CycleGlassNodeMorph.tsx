...
const frameSeg = (frame: number, start: number, end: number, ease: (t: number) => number = E.linear) => ease(Math.min(1, Math.max(0, (frame - start) / Math.max(1, end - start))));
...
const Marker: React.FC<{
    ...
    top: number;
    rotate: number;
}> = ({ frame, start, end, left, top, rotate, }) => {
    const k = frameSeg(frame, start, end, E.outBack);
    return (<PvElement as={"div"} style={{
            position: 'absolute',
            left,
...
export const CycleGlassNodeMorph: React.FC = () => {
    ...
    return (<DesignStage bg="#f5f5f2" raster="zoom">
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"svg"} viewBox="0 0 332 134" style={{
            position: 'absolute',
            left: 240,
            top: 154,
          ...
          <PvElement as={"polygon"} points="56,27 96,0 236,0 276,27 332,47 332,134 0,134 0,47" fill="#5a5a5d"/>
          <PvElement as={"text"} x="166" y="82" textAnchor="middle" dominantBaseline="middle" fill="#fff" fontFamily="Arial, sans-serif" fontSize="18" fontWeight="850">
            SUBJECT
          </PvElement>
        </PvElement>
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"svg"} viewBox="0 0 370 226" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
            <PvElement as={"path"} d="M68 98 Q185 4 302 98" fill="none" stroke="#4d4d50" strokeWidth="1.3" strokeDasharray="300" strokeDashoffset={300 * (1 - arrows)}/>
            <PvElement as={"path"} d="M302 100 Q185 190 68 100" fill="none" stroke="#4d4d50" strokeWidth="1.3" strokeDasharray="300" strokeDashoffset={300 * (1 - arrows)}/>
            <PvElement as={"path"} d="M-9 -5 L0 0 L-9 5" fill="none" stroke="#4d4d50" strokeWidth="1.3" opacity={frameSeg(arrows, 0.02, 0.08, E.outQuad)} transform={`translate(${topArrow.x} ${topArrow.y}) rotate(${topArrow.angle})`}/>
            <PvElement as={"path"} d="M-9 -5 L0 0 L-9 5" fill="none" stroke="#4d4d50" strokeWidth="1.3" opacity={frameSeg(arrows, 0.02, 0.08, E.outQuad)} transform={`translate(${bottomArrow.x} ${bottomArrow.y}) rotate(${bottomArrow.angle})`}/>
          </PvElement>
          ...
          {labelData.map((item, i) => {
            ...
            return (<PvElement as={"div"} key={item.label} style={{
                    position: 'absolute',
