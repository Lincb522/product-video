...
export const BrandFrameSnap: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: '#161618', position: 'relative', overflow: 'hidden' }}>
      {/* 框内内容区 */}
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"div"} style={{
            ...
            {[0, 1, 2].map((i) => (<PvElement as={"div"} key={i} style={{ width: 16, height: 16, borderRadius: 8, background: G.bar }}/>))}
            <PvElement as={"div"} style={{ marginLeft: 18, height: 12, width: 260, background: G.line, borderRadius: 6 }}/>
            {/* 模式徽标：随画框同帧换色换字 */}
            <PvElement as={"div"} style={{
          ...
          {/* 窗口内容：翻色同帧换布局 A→B */}
          <PvElement as={"div"} style={{ transform: 'scale(0.81)', transformOrigin: '0 0', width: 1920, height: 1080 }}>
      ...
      {([
        ...
        ] as React.CSSProperties[]).map((pos, i) => (<PvElement as={"div"} key={i} style={{ position: 'absolute', background: frameColor, ...pos }}/>))}
      ...
      {/* 翻色白闪脉冲 */}
      {flash > 0 && (<PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: '#ffffff', opacity: flash }}/>)}
    </PvElement>);
};
