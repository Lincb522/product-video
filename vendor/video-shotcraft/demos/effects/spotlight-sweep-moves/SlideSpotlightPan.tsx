...
const SideRow: React.FC<{
    w: number;
}> = ({ w }) => (<PvElement as={"div"} style={{ display: 'flex', alignItems: 'center', gap: 16, height: 44 }}>
    <PvElement as={"div"} style={{ width: 26, height: 26, borderRadius: 7, background: mid }}/>
    <PvElement as={"div"} style={{ height: 15, width: w, background: '#c6c6c4', borderRadius: 7 }}/>
  </PvElement>);
const Task: React.FC<{
    seed: number;
}> = ({ seed }) => (<PvElement as={"div"} style={{ display: 'flex', flexDirection: 'column', gap: 13, padding: '26px 0' }}>
    <PvElement as={"div"} style={{ height: 13, width: 210 + (seed % 3) * 30, background: line, borderRadius: 6 }}/>
    <PvElement as={"div"} style={{ height: 17, width: 260 + ((seed * 7) % 4) * 26, background: '#aeaeac', borderRadius: 8 }}/>
    <PvElement as={"div"} style={{ width: 22, height: 16, background: '#d6d6d4', borderRadius: 3 }}/>
  </PvElement>);
const Col: React.FC<{
    accent: string;
    seed: number;
    w: number;
}> = ({ accent, seed, w }) => (<PvElement as={"div"} style={{ width: w, flexShrink: 0 }}>
    <PvElement as={"div"} style={{ borderTop: `6px solid ${accent}`, paddingTop: 24, display: 'flex', alignItems: 'center', gap: 14 }}>
      <PvElement as={"div"} style={{ height: 20, width: 120, background: '#525250', borderRadius: 9 }}/>
      <PvElement as={"div"} style={{ width: 32, height: 32, borderRadius: 16, border: `3px solid ${line}` }}/>
      <PvElement as={"div"} style={{ marginLeft: 'auto', width: 22, height: 22, background: line, borderRadius: 4 }}/>
    </PvElement>
    {[0, 1].map((i) => <Task key={i} seed={seed + i}/>)}
  </PvElement>);
// 超宽面板内容（放大特写级别）：侧栏 + 顶栏 + 三列看板
const WidePanel: React.FC = () => (<PvElement as={"div"} style={{
        width: PW, height: PH, background: '#f4f4f3',
        display: 'flex', boxSizing: 'border-box',
        borderRadius: `${CR}px ${CR}px 0 0`,
    }}>
    <PvElement as={"div"} style={{ width: 560, borderRight: `3px solid ${line}`, padding: '52px 48px', boxSizing: 'border-box' }}>
      <PvElement as={"div"} style={{ display: 'flex', alignItems: 'center', gap: 18, marginBottom: 56 }}>
        <PvElement as={"div"} style={{ width: 46, height: 46, borderRadius: 12, background: ink }}/>
        <PvElement as={"div"} style={{ height: 26, width: 150, background: ink, borderRadius: 10 }}/>
      </PvElement>
      <PvElement as={"div"} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <SideRow w={110}/><SideRow w={200}/><SideRow w={100}/>
      </PvElement>
      <PvElement as={"div"} style={{ height: 20, width: 130, background: '#adadab', borderRadius: 9, margin: '58px 0 24px' }}/>
      <PvElement as={"div"} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <SideRow w={180}/><SideRow w={220}/><SideRow w={170}/>
      </PvElement>
    </PvElement>
    <PvElement as={"div"} style={{ flex: 1, padding: '52px 64px', boxSizing: 'border-box' }}>
      <PvElement as={"div"} style={{ display: 'flex', alignItems: 'center', gap: 22, marginBottom: 48 }}>
        <PvElement as={"div"} style={{ width: 36, height: 30, background: mid, borderRadius: 6 }}/>
        <PvElement as={"div"} style={{ height: 26, width: 400, background: '#606060', borderRadius: 11 }}/>
        <PvElement as={"div"} style={{ height: 18, width: 150, background: line, borderRadius: 8, marginLeft: 64 }}/>
        <PvElement as={"div"} style={{ height: 18, width: 120, background: line, borderRadius: 8 }}/>
        <PvElement as={"div"} style={{ height: 18, width: 90, background: line, borderRadius: 8 }}/>
      </PvElement>
      <PvElement as={"div"} style={{ display: 'flex', gap: 84 }}>
...
export const SlideSpotlightPan: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: '#050409', overflow: 'hidden' }}>
      {/* 面板层：聚光范围内显影 */}
      <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: TOP, transform: `translateX(${slide}px)` }}>
        <WidePanel />
      ...
      <PvElement as={"div"} style={{ opacity: hGlow }}>
        <PvElement as={"div"} style={{
      ...
      {/* 左上角竖缘光线（起始阶段，贴面板左缘） */}
      <PvElement as={"div"} style={{ opacity: vGlow }}>
        <PvElement as={"div"} style={{
