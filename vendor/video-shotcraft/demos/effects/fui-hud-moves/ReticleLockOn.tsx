...
export const ReticleLockOn: React.FC = () => {
    ...
    const shrink = interpolate(frame, [FLY_END, LOCK_END - 8, LOCK_END - 3, LOCK_END], [2.2, 1.06, 0.94, 1], { easing: Easing.inOut(Easing.cubic), ...clamp });
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, overflow: 'hidden', position: 'relative' }}>
      ...
      {showReticle && (<PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, transform: `translate(${flyDX}px, ${flyDY}px)` }}>
          {corners.map((c, i) => (<PvElement as={"div"} key={i} style={{
                    position: 'absolute',
                    left: c.x,
                    top: c.y,
              ...
              {/* L 形角标：横臂 + 竖臂 */}
              <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, width: ARM, height: THICK, background: G.ink }}/>
              <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, width: THICK, height: ARM, background: G.ink }}/>
            </PvElement>))}
          {/* 小标签：右上角标外侧弹出 */}
          {frame >= LOCK_END + 2 && (<PvElement as={"div"} style={{
                    position: 'absolute',
                    left: TCX + hw + 18,
                    top: TCY - hh - 6,
