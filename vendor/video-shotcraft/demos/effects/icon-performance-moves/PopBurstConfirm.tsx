...
export const PopBurstConfirm: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      {/* 扩散圆环（不随 icon 缩放） */}
      {rt > 0 && rt < 1 && (<PvElement as={"svg"} width={1400} height={1400} style={{ position: 'absolute', left: CX - 700, top: CY - 700 }}>
          <PvElement as={"circle"} cx={700} cy={700} r={ringR} fill="none" stroke={AMBER} strokeWidth={ringW} opacity={ringO}/>
        </PvElement>)}

      {/* 粒子：10 根短线径向飞出，角度带 sin 散列微抖 */}
      {pt > 0 && pt < 1 && (<PvElement as={"svg"} width={1400} height={1400} style={{ position: 'absolute', left: CX - 700, top: CY - 700 }}>
          {Array.from(pvArrayCopy({ length: 10 })).map((_, i) => {
                ...
                return (<PvElement as={"line"} key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={i % 5 < 2 ? AMBER : G.ink} strokeWidth={12} strokeLinecap="round" opacity={1 - pt}/>);
            })}
        </PvElement>)}

      {/* 主体 icon：圆底 + 对勾（半屏特写 ~480px） */}
      <PvElement as={"svg"} width={480} height={480} viewBox="0 0 480 480" style={{
            position: 'absolute',
            left: CX - 240,
            top: CY - 240,
        ...
        <PvElement as={"circle"} cx={240} cy={240} r={190} fill={G.card} stroke={G.ink} strokeWidth={22}/>
        {checkT > 0 && (<PvElement as={"path"} d="M 150 245 L 215 310 L 340 175" fill="none" stroke={AMBER} strokeWidth={34} strokeLinecap="round" strokeLinejoin="round" pathLength={1} strokeDasharray={1} strokeDashoffset={1 - checkT}/>)}
      </PvElement>

      {/* Deployed 小标签 */}
      {tagT > 0 && (<PvElement as={"div"} style={{
                position: 'absolute',
                left: CX - 130,
                top: CY + 300,
                width: 260,
