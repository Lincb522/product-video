...
export const AttentionBounce: React.FC = () => {
    ...
    // 起跳前的预压（第一跳前 3f 蹲一下）
    if (f >= 9 && f < 12)
        squash = Math.max(squash, (f - 9) / 3 * 0.7);
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        {/* 地面线 */}
        <PvElement as={"div"} style={{ position: 'absolute', left: 120, top: GROUND, width: 1680, height: 8, background: G.bar, borderRadius: 4 }}/>

        {/* 落地尘点 */}
        {dusts.map((d, i) => (<PvElement as={"div"} key={i} style={{
                position: 'absolute',
                left: d.x - d.r,
                top: d.y - d.r,
                width: d.r * 2,
        ...
        <PvElement as={"svg"} width={ICON} height={ICON} viewBox="0 0 420 420" style={{
            position: 'absolute',
            left: CX - ICON / 2,
            top: iconTop,
          ...
          <PvElement as={"rect"} x={14} y={14} width={392} height={392} rx={88} fill={G.card} stroke={G.ink} strokeWidth={18}/>
          {/* 符号：简洁铃形 */}
          <PvElement as={"path"} d="M 210 110 C 160 110 140 155 138 200 C 136 245 118 272 100 290 L 320 290 C 302 272 284 245 282 200 C 280 155 260 110 210 110 Z" fill="none" stroke={G.ink} strokeWidth={22} strokeLinejoin="round"/>
          <PvElement as={"circle"} cx={210} cy={322} r={22} fill={AMBER}/>
        </PvElement>

        {/* 功能面板卡：落定后弹出 */}
        {panelT > 0 && (<PvElement as={"div"} style={{
                position: 'absolute',
                left: CX + ICON / 2 + 60,
                top: GROUND - ICON - 40,
