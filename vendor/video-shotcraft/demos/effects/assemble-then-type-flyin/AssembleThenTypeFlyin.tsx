...
let charSeed = 0;
const CHAR_PARAMS: CharP[][][] = BLOCKS.map((b) => b.segs.map((sg) => Array.from(sg.s, () => {
    const k = charSeed++;
    return {
        dx: (rand(k) - 0.5) * 340,
...
export const AssembleThenTypeFlyin: React.FC = () => {
    ...
    return (<DesignStage bg="#0a0b0e">
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,#101116,#0c0d11)' }}>
        ...
        {/* 顶栏 url pill + 右侧短线 */}
        <PvElement as={"div"} style={{ ...shell(SH.urlPill), left: 18, top: 11, width: 78, height: 14, border: '1px solid #2a2c33', borderRadius: 9 }}/>
        <PvElement as={"div"} style={{ ...shell(SH.topLine), right: 18, top: 16, width: 52, height: 5, background: '#1d1f26', borderRadius: 2 }}/>
        {/* logo 点阵 */}
        <PvElement as={"div"} style={{ ...shell(SH.mark), left: 24, top: 37, width: 10, height: 10 }}>
          {[0, 1, 2, 3].map((i) => (<PvElement as={"div"} key={i} style={{
                position: 'absolute',
                width: 4,
                height: 4,
                borderRadius: '50%',
        ...
        {/* CTA pill 骨架 */}
        <PvElement as={"div"} style={{ ...shell(SH.cta), left: 22, top: 166, width: 74, height: 24, border: '1px solid #3a3d46', borderRadius: 12 }}/>
        {/* 社交 x 方块 */}
        <PvElement as={"div"} style={{ ...shell(SH.social), left: 352, top: 241, width: 11, height: 11, border: '1px solid #3a3d46', borderRadius: 2 }}/>
        ...
        {BLOCKS.map((b, bi) => {
            ...
            return (<PvElement as={"div"} key={bi} style={{
                    position: 'absolute',
                    left: b.x,
                    top: b.y,
                    font: b.font,
              ...
              {b.segs.map((sg, si) => (<PvElement as={"span"} key={si} style={sg.i ? { fontStyle: 'italic' } : undefined}>
                  {Array.from(pvArrayCopy(sg.s), (ch, k) => {
                        const c = CHAR_PARAMS[bi][si][k];
                        const ft = b.start + ci++ * step;
                        const a = seg(t, ft, ft + 0.13, E.outCubic);
                        return (<PvElement as={"span"} key={k} style={{
