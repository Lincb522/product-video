...
const TitleText: React.FC = () => (<PvElement as={AbsoluteFill} style={{ justifyContent: 'center', alignItems: 'center' }}>
    <PvElement as={"div"} style={textStyle}>{'SPOTLIGHT\nON'}</PvElement>
  </PvElement>);
export const SpotlightSweepReveal: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: BG }}>
      {/* 暗版文字常驻 */}
      <PvElement as={AbsoluteFill} style={{ opacity: 0.07 }}>
        <TitleText />
      </PvElement>
      ...
      {/* 全亮定格层 */}
      <PvElement as={AbsoluteFill} style={{ opacity: brighten }}>
        <TitleText />
      </PvElement>
    </PvElement>);
};
