...
const SpeedLines: React.FC<{
    ...
    const polys = Array.from(pvArrayCopy({ length: 30 })).map((_, i) => {
    ...
    return (<PvElement as={"svg"} viewBox="0 0 1920 1080" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
      {polys.map((pts, i) => (<PvElement as={"polygon"} key={i} points={pts} fill={i % 4 === 0 ? '#111111' : '#f5f5f5'}/>))}
    </PvElement>);
};
// 目标卡叠层 + 全景底
const Scene: React.FC = () => (<>
    <FakeDashboard variant="A"/>
    <PvElement as={"div"} style={{ position: 'absolute', left: CARD.x, top: CARD.y }}>
      <Card w={CARD.w} h={CARD.h} seed={9} style={{ boxShadow: '0 10px 36px rgba(0,0,0,0.18)', border: `3px solid ${G.ink}` }}/>
      <PvElement as={"div"} style={{ position: 'absolute', left: 24, bottom: 96 }}>
...
export const AnimeImpact: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: impact ? '#131315' : G.bg, overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, transform: `translate(${shakeX}px, ${shakeY}px)` }}>
        {/* 主层：冲击帧期间整幅负片黑白 */}
        <PvElement as={"div"} style={{ ...zoomStyle, filter: impact ? 'invert(1) grayscale(1)' : 'none' }}>
          <Scene />
        </PvElement>
        ...
        {impact && (<>
            <PvElement as={"div"} style={{
              ...
              <PvElement as={"div"} style={{ ...zoomStyle, filter: 'invert(1) grayscale(1)' }}>
                <Scene />
              </PvElement>
              <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: '#ff0033', mixBlendMode: 'multiply' }}/>
            </PvElement>
            <PvElement as={"div"} style={{
                position: 'absolute',
                inset: 0,
              ...
              <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: '#00e5ff', mixBlendMode: 'multiply' }}/>
            </PvElement>
            {/* 手绘放射集中线：每帧换形态 */}
