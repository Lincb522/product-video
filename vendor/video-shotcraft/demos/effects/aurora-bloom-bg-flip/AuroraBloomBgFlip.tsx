...
const mix = (a: number[], b: number[], k: number) => `rgb(${Math.round(a[0] + (b[0] - a[0]) * k)},${Math.round(a[1] + (b[1] - a[1]) * k)},${Math.round(a[2] + (b[2] - a[2]) * k)})`;
...
export const AuroraBloomBgFlip: React.FC = () => {
    ...
    return (<DesignStage bg={mix(LIGHT, DARK, flip)}>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ position: 'absolute' }}>
          {WA.map((w, i) => {
