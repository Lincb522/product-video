...
const Flyline: React.FC<{
    ...
    if (frame < start || frame >= start + DUR + HOLD + FADE)
        return null; // 条件挂载=摘罩
    ...
    for (let i = 0; i < pts.length - 1; i++) {
        ...
        segs.push(<PvElement as={"line"} key={i} x1={pts[i].x} y1={pts[i].y} x2={pts[i + 1].x} y2={pts[i + 1].y} stroke="#f4f4f0" strokeWidth={5} strokeLinecap="round" strokeOpacity={grad * fade}/>);
    }
    return (<PvElement as={"g"}>
      {/* 宽幅低透明白 = 辉光衬底（暗底上可见） */}
      <PvElement as={"polyline"} points={poly} fill="none" stroke="#e8e8e4" strokeWidth={14} strokeLinecap="round" strokeLinejoin="round" strokeOpacity={0.18 * fade}/>
      {segs}
      {/* 亮点头领跑：仅生长期挂载 */}
      {growing && (<PvElement as={"g"}>
          <PvElement as={"circle"} cx={head.x} cy={head.y} r={34} fill={`url(#${haloId})`}/>
          <PvElement as={"circle"} cx={head.x} cy={head.y} r={8} fill="#ffffff"/>
        </PvElement>)}
    </PvElement>);
};
...
const DarkCard: React.FC<{
    ...
    return (<PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{ height: 15, width: `${titleW}%`, background: '#4a4a48', borderRadius: 8 }}/>
      <PvElement as={"div"} style={{ height: 10, width: '80%', background: '#383836', borderRadius: 5 }}/>
      <PvElement as={"div"} style={{ height: 10, width: '62%', background: '#383836', borderRadius: 5 }}/>
      <PvElement as={"div"} style={{ marginTop: 'auto', display: 'flex', gap: 9, alignItems: 'center' }}>
        <PvElement as={"div"} style={{ width: 24, height: 24, borderRadius: 12, background: '#4a4a48' }}/>
        <PvElement as={"div"} style={{ height: 10, width: 78, background: '#383836', borderRadius: 5 }}/>
      </PvElement>
    </PvElement>);
};
export const OrbFlylineRelay: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: '#1d1d1b', overflow: 'hidden' }}>
      ...
      {/* 飞线接力层 */}
      <PvElement as={"svg"} width={1920} height={1080} viewBox="0 0 1920 1080" style={{ position: 'absolute', top: 0, left: 0, pointerEvents: 'none' }}>
        <PvElement as={"defs"}>
          <PvElement as={"radialGradient"} id={haloId}>
            <PvElement as={"stop"} offset="0%" stopColor="rgba(255,255,255,0.55)"/>
            <PvElement as={"stop"} offset="55%" stopColor="rgba(255,255,255,0.22)"/>
            <PvElement as={"stop"} offset="100%" stopColor="rgba(255,255,255,0)"/>
          </PvElement>
        </PvElement>
