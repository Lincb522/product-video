...
const Flyline: React.FC<{
    ...
    for (let i = 0; i < pts.length - 1; i++) {
        ...
        segs.push(<PvElement as={"line"} key={i} x1={pts[i].x} y1={pts[i].y} x2={pts[i + 1].x} y2={pts[i + 1].y} stroke="#fafafa" strokeWidth={4.5} strokeLinecap="round" strokeOpacity={op}/>);
    }
    return (<PvElement as={"g"}>
      {/* 深色衬底：保证浅底可见 */}
      <PvElement as={"polyline"} points={underlay} fill="none" stroke="#2f2f2f" strokeWidth={9} strokeLinecap="round" strokeLinejoin="round" strokeOpacity={0.92}/>
      {segs}
      {/* 光头：仅生长期挂载，摘罩即真静止 */}
      {growing && (<PvElement as={"g"}>
          <PvElement as={"circle"} cx={head.x} cy={head.y} r={26} fill={`url(#${haloId})`}/>
          <PvElement as={"circle"} cx={head.x} cy={head.y} r={9} fill="#ffffff" stroke="#2f2f2f" strokeWidth={3}/>
        </PvElement>)}
    </PvElement>);
};
...
const CardPulse: React.FC<{
    ...
    return (<PvElement as={"g"}>
      <PvElement as={"rect"} x={rect.x + 2} y={rect.y + 2} width={rect.w - 4} height={rect.h - 4} rx={14} fill={`rgba(0,0,0,${(0.16 * amp).toFixed(3)})`}/>
      <PvElement as={"rect"} x={rect.x + 2} y={rect.y + 2} width={rect.w - 4} height={rect.h - 4} rx={14} fill="none" stroke="#2f2f2f" strokeWidth={4} strokeOpacity={amp}/>
    </PvElement>);
};
export const FlylineArc: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, position: 'relative', background: '#e0e0de' }}>
      {/* 背景微暗化 0.92，常量、不动画 */}
      <PvElement as={"div"} style={{ filter: 'brightness(0.92)' }}>
      ...
      <PvElement as={"svg"} width={1920} height={1080} viewBox="0 0 1920 1080" style={{ position: 'absolute', top: 0, left: 0, pointerEvents: 'none' }}>
        <PvElement as={"defs"}>
          <PvElement as={"radialGradient"} id={haloId}>
            <PvElement as={"stop"} offset="0%" stopColor="rgba(0,0,0,0.38)"/>
            <PvElement as={"stop"} offset="55%" stopColor="rgba(0,0,0,0.18)"/>
            <PvElement as={"stop"} offset="100%" stopColor="rgba(0,0,0,0)"/>
          </PvElement>
        </PvElement>
