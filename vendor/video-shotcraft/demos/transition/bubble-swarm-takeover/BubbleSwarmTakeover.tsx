...
const Bubble: React.FC<{
    ...
    const opacity = interpolate(frame, [spec.t0, spec.t0 + 7], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }) *
...
export const BubbleSwarmTakeover: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: '#ececea', overflow: 'hidden' }}>
      <PvElement as={AbsoluteFill} style={{ transform: `scale(${breathe})` }}>
        {frame < PEAK ? <FakeDashboard variant="A"/> : <FakeDashboard variant="B"/>}
      </PvElement>
      {/* 洗白层压在页面之上、气泡之下 */}
      <PvElement as={AbsoluteFill} style={{ background: '#ffffff', opacity: whiteout }}/>
      {/* 远/中层气泡 */}
      {BUBBLES.filter((b) => b.z < 2).map((b, i) => (<Bubble key={i} spec={b} frame={frame}/>))}
      {/* i18n 文字胶囊混在中层气泡里漂 */}
      {CAPSULES.map((c, i) => {
