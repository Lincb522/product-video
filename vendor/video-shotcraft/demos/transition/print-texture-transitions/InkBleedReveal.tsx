...
export const InkBleedReveal: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      {/* 旧景：纸底 + BEFORE 标题 */}
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: G.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <TitleBlock text="BEFORE" size={120}/>
      </PvElement>

      {settled ? (<PvElement as={"div"} style={{ position: 'absolute', inset: 0 }}>
          <FakeDashboard variant="A"/>
        </PvElement>) : (<PvElement as={"svg"} width={1920} height={1080} viewBox="0 0 1920 1080" style={{ position: 'absolute', inset: 0, display: 'block' }}>
          <PvElement as={"defs"}>
            {/* filter 只挂在 mask 的圆上——揉的是遮罩边，不是画面内容 */}
            <PvElement as={"filter"} id={bleedId} x="-40%" y="-40%" width="180%" height="180%">
              <PvElement as={"feTurbulence"} type="fractalNoise" baseFrequency="0.02" numOctaves="3" seed="7" result="noise"/>
              <PvElement as={"feDisplacementMap"} in="SourceGraphic" in2="noise" scale={dispScale} xChannelSelector="R" yChannelSelector="G"/>
            </PvElement>
            <PvElement as={"mask"} id={maskId} maskUnits="userSpaceOnUse" x="0" y="0" width="1920" height="1080">
              <PvElement as={"rect"} x="0" y="0" width="1920" height="1080" fill="black"/>
              {r > 0.5 && <PvElement as={"circle"} cx={cx} cy={cy} r={r} fill="white" filter={`url(#${bleedId})`}/>}
            </PvElement>
          </PvElement>
          <PvElement as={"g"} mask={`url(#${maskId})`}>
            <PvElement as={"foreignObject"} x="0" y="0" width="1920" height="1080">
              <FakeDashboard variant="A"/>
            </PvElement>
          </PvElement>
        </PvElement>)}
