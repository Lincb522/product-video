...
const Scene: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: G.bg, overflow: 'hidden' }}>
      {/* 背景层：硬切藏在遮挡帧内 */}
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, transform: `translateX(${shove}px)` }}>
        {frame < CUT ? <FakeDashboard variant="A"/> : <FakeDashboard variant="B"/>}
      </PvElement>
      {/* 手动残影：4 层拖尾（在主卡身后），保证遮挡窗口糊满全屏 */}
      {sweeping &&
            [4, 3, 2, 1].map((i) => (<PvElement as={"div"} key={i} style={{
                    position: 'absolute',
...
export const InvisibleCut: React.FC = () => (<CameraMotionBlur shutterAngle={300} samples={12}>
    <Scene />
  </CameraMotionBlur>);
