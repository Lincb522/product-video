...
export const VersusSlam: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: G.bg, overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, transform: `translate(${shakeX}px, ${shakeY}px)` }}>
        {/* 建立段的斜缝虚线预示（撞合后被实缝替代） */}
        {!impacted && (<PvElement as={"svg"} width={1920} height={1080} style={{ position: 'absolute', inset: 0 }}>
            <PvElement as={"line"} x1={SEAM_TOP_X} y1={0} x2={SEAM_BOT_X} y2={1080} stroke={G.bar} strokeWidth={4} strokeDasharray="18 16"/>
          </PvElement>)}
        ...
        {/* 撞合后的实体斜缝条 */}
        {impacted && (<PvElement as={"div"} style={{
      ...
      {/* 撞击白闪（不随震屏位移） */}
      <PvElement as={AbsoluteFill} style={{ background: '#ffffff', opacity: flash, pointerEvents: 'none' }}/>
    </PvElement>);
};
