...
const WIN: [
    number,
    number
][] = [[0.10, 0.24], [0.30, 0.44], [0.50, 0.62], [0.66, 0.78], [0.84, 0.97]];
type Cam = {
    rx: number;
    ry: number;
    d: number;
...
const acc = (t: number, base: Cam, kfs: {
    at: [
        number,
        number
    ];
    to: Cam;
}[], ease: (x: number) => number): Cam => {
    const out: Cam = { ...base };
    let prev = base;
    for (const kf of kfs) {
...
export const CubeNavigation: React.FC = () => {
    ...
    return (<DesignStage bg="#000">
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{
          ...
          {FACES.map((f, i) => {
            ...
            return (<PvElement as={"div"} key={i} style={{
                ...
                {Array.from(pvArrayCopy({ length: 4 }), (_, k) => (<PvElement as={"div"} key={k} style={{
                        position: 'absolute',
                        left: 14,
                        top: 42 + k * 20,
                        height: 9,
        ...
        {rCrush > 0 && (<PvElement as={"div"} style={{
                position: 'absolute',
                inset: 0,
