...
const Window: React.FC<{
    ...
    sendBtn?: boolean;
    btnPulse?: number;
}> = ({ cx, cy, seed, sendBtn, btnPulse = 0 }) => (<PvElement as={"div"} style={{ position: 'absolute', left: cx - WIN_W / 2, top: cy - WIN_H / 2, width: WIN_W, height: WIN_H, borderRadius: 18, background: '#fff', border: `2px solid ${G.border}`, boxShadow: '0 40px 100px rgba(0,0,0,0.30)', boxSizing: 'border-box', overflow: 'hidden' }}>
    <PvElement as={"div"} style={{ height: 54, background: '#f2f2f0', borderBottom: `2px solid ${G.line}`, display: 'flex', alignItems: 'center', gap: 10, padding: '0 20px', boxSizing: 'border-box' }}>
      {[0, 1, 2].map((i) => <PvElement as={"div"} key={i} style={{ width: 15, height: 15, borderRadius: 8, background: G.bar }}/>)}
      <PvElement as={"div"} style={{ marginLeft: 14, height: 13, width: 200, background: G.bar, borderRadius: 7 }}/>
    </PvElement>
    <PvElement as={"div"} style={{ padding: 26, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <Card w={WIN_W - 56} h={250} seed={seed} style={{ boxShadow: 'none' }}/>
      {sendBtn && (<PvElement as={"div"} style={{ alignSelf: 'flex-end', transform: `scale(${1 + btnPulse * 0.22})`, padding: '16px 46px', borderRadius: 12, background: G.ink, color: '#fff', fontFamily: 'Helvetica, Arial, sans-serif', fontWeight: 800, fontSize: 30, boxShadow: btnPulse > 0 ? `0 0 0 ${btnPulse * 22}px rgba(47,47,47,0.18)` : 'none' }}>
          Send ➤
        </PvElement>)}
    </PvElement>
  </PvElement>);
const Plane: React.FC<{
    ...
    scale: number;
    opacity: number;
}> = ({ x, y, angle, scale, opacity }) => (<PvElement as={"svg"} width={180} height={110} viewBox="0 0 180 110" style={{ position: 'absolute', left: x - 90, top: y - 55, transform: `rotate(${angle}deg) scale(${scale})`, overflow: 'visible', opacity }}>
    {/* 折纸飞机：三块折面，灰阶深浅示折痕 */}
    <PvElement as={"polygon"} points="176,30 6,4 62,66" fill="#ffffff" stroke="#b8b8b6" strokeWidth={3}/>
    <PvElement as={"polygon"} points="176,30 62,66 78,102" fill="#d4d4d2" stroke="#b8b8b6" strokeWidth={3}/>
    <PvElement as={"polygon"} points="176,30 6,4 50,44" fill="#efefed" stroke="#c6c6c4" strokeWidth={2}/>
  </PvElement>);
export const PaperPlaneMessenger: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: 'linear-gradient(160deg, #e9e9e7 0%, #dcdcda 100%)', overflow: 'hidden' }}>
      {/* 视差道具：远层 / 中层（世界层之下） */}
      ...
      <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, transformOrigin: '0 0', transform: `translate(${960 - cx * z}px, ${540 - cy * z}px) scale(${z})` }}>
