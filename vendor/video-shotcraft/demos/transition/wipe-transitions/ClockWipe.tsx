...
export const ClockWipe: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: '#ececea' }}>
      ...
      {!wipeDone && (<PvElement as={AbsoluteFill}>
          <FakeDashboard variant="A"/>
        </PvElement>)}

      {/* 上层：B 页。扫描期挂扇形 clip-path，擦完后摘罩直出 */}
      {frame >= 30 && (<PvElement as={AbsoluteFill} style={wipeDone ? undefined : { clipPath: fanClip(theta) }}>
          <FakeDashboard variant="B"/>
        </PvElement>)}

      {/* 扫描亮线：柔光 + 暗描边 + 白核，从屏心指向当前角度 */}
      {lineMounted && (<PvElement as={"svg"} width={1920} height={1080} style={{ position: 'absolute', inset: 0, opacity: lineOpacity, pointerEvents: 'none' }}>
          {/* 柔光带（QA 后加码 1.5x：白底看不清就加深+加宽） */}
          <PvElement as={"line"} x1={CX} y1={CY} x2={x2} y2={y2} stroke="rgba(255,255,255,0.35)" strokeWidth={26} strokeLinecap="round"/>
          <PvElement as={"line"} x1={CX} y1={CY} x2={x2} y2={y2} stroke="rgba(255,255,255,0.60)" strokeWidth={13} strokeLinecap="round"/>
          {/* 暗描边：白底上"提亮"不可见，加深保证浅色区也读得出指针 */}
          <PvElement as={"line"} x1={CX} y1={CY} x2={x2} y2={y2} stroke="rgba(0,0,0,0.55)" strokeWidth={9} strokeLinecap="round"/>
          {/* 白核 */}
          <PvElement as={"line"} x1={CX} y1={CY} x2={x2} y2={y2} stroke="rgba(255,255,255,0.95)" strokeWidth={4} strokeLinecap="round"/>
        </PvElement>)}
    </PvElement>);
};
