...
const Scene: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: '#0c0c10', overflow: 'hidden' }}>
      {/* A 景 */}
      {frame < 14 ? (<PvElement as={"div"} style={{ position: 'absolute', opacity: aOut }}>
          <PvElement as={Img} src={staticFile('textures/live/projects-full.png')} style={{ position: 'absolute', left: 0, top: A_VIEW_Y, width: 1920 }}/>
        </PvElement>) : null}

      {/* 黑场字卡（18–74） */}
      {frame >= 18 && frame < 74 ? (<PvElement as={AbsoluteFill} style={{
