...
const Scene: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: '#faf7f2', overflow: 'hidden' }}>
      {/* A 景：同一页的上半部（网格区） */}
      <PvElement as={"div"} style={{ position: 'absolute', opacity: 1 - aOut, filter: `blur(${aBlur}px)`, transform: `translateX(${-aDrift}px)` }}>
        <PvElement as={Img} src={staticFile('textures/live/projects-full.png')} style={{ position: 'absolute', left: 0, top: A_VIEW_Y, width: 1920, height: PAGE_H }}/>
      </PvElement>

      {/* B 景（上层）：同一页的下半部（sections 区），交叉窗口内收焦 */}
      <PvElement as={"div"} style={{ position: 'absolute', opacity: bIn, filter: `blur(${bBlur}px)`, transform: `translateX(${bDrift}px)` }}>
        <PvElement as={Img} src={staticFile('textures/live/projects-full.png')} style={{ position: 'absolute', left: 0, top: B_VIEW_Y, width: 1920, height: PAGE_H }}/>
      </PvElement>
    </PvElement>);
};
export const FocusHandoffTransition: React.FC = () => (<Scene />);
