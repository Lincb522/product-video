...
const Scene: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: '#f9f6f1', overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ position: 'absolute', transform: `translateX(${-dx}px)` }}>
        {/* A 景：项目板全满 */}
        <PvElement as={Img} src={staticFile('textures/live/projects-full.png')} style={{ position: 'absolute', left: 0, top: VIEW_Y, width: 1920 }}/>
        {/* 缝隙纸色空场 960px——甩动中段一晃而过，全糊 */}
        {/* B 景：同页底部 section（PERCEPTION & SENSING 区），换景可辨且内容饱满 */}
        <PvElement as={Img} src={staticFile('textures/live/projects-full.png')} style={{ position: 'absolute', left: SWING, top: -666, width: 1920 }}/>
      </PvElement>
    </PvElement>);
};
