...
const Scene: React.FC = () => {
    ...
    const DUST = Array.from(pvArrayCopy({ length: 4 }), (_, i) => ({
        x: 780 + i * 320,
        y: 300 + ((i * 137) % 480),
        drift: 6 + (i % 3) * 4, // px/frame，缓慢漂移表示"still moving"
    }));
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: '#0d0d10', overflow: 'hidden' }}>
      {/* A 景（推出段） */}
      {frame < 42 ? (<PvElement as={"div"} style={{ position: 'absolute', transform: `translateX(${pushX}px)` }}>
          <PvElement as={Img} src={staticFile('textures/live/projects-full.png')} style={{ position: 'absolute', left: 0, top: A_VIEW_Y, width: A_W }}/>
        </PvElement>) : null}

      {/* 暗场：微渐变 + 尘点，非纯黑死场 */}
      {frame >= 32 && frame < 50 ? (<PvElement as={AbsoluteFill} style={{ opacity: tunnelVis, pointerEvents: 'none' }}>
          <PvElement as={"div"} style={{
                position: 'absolute', inset: 0,
                background: 'radial-gradient(900px 500px at 50% 45%, rgba(40,42,50,0.9), rgba(12,12,16,1) 70%)',
            }}/>
          {DUST.map((d, i) => (<PvElement as={"div"} key={i} style={{
      ...
      {frame >= 42 ? (<PvElement as={"div"} style={{
          ...
          <PvElement as={Img} src={staticFile('textures/live/wbr-full.png')} style={{ position: 'absolute', left: 0, top: 0, width: 1920, height: 1080 }}/>
        </PvElement>) : null}
    </PvElement>);
};
export const DarkTunnelTransition: React.FC = () => (<Scene />);
