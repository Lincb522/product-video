...
export const SpotlightHeroCard: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: '#faf7f2' }}>
      <PvElement as={AbsoluteFill} style={{ opacity: macroIn }}>
        <PageCam2D src="textures/live/projects-full.png" pageH={PAGE_H} keys={CAM_KEYS} ease={PUSH_EASE}>
          ...
          <PvElement as={"div"} style={{ transformStyle: 'preserve-3d' }}>
            {/* vacated-slot patch + breathing amber outline */}
            {slotVis > 0.02 ? (<PvElement as={"div"} style={{
            ...
            boxShadow: landed ? 'none' : shadow,
                ...
                <PvElement as={Img} src={staticFile(`textures/live/${CARD.file}`)} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }}/>
                {/* 4x hi-res capture layered on top, crisp under push-in zoom */}
                <PvElement as={Img} src={staticFile('textures/live/card4-hires.png')} style={{
            ...
            boxShadow: `inset 0 0 0 1px rgba(255,255,255,${0.7 * lift})`, pointerEvents: 'none',
              ...
              {(beam1On || beam2On) && lift > 0.4 ? (<PvElement as={"svg"} width={bw} height={bh} viewBox={`0 0 ${bw} ${bh}`} style={{
                ...
                filter: `drop-shadow(0 0 6px ${AMBER}) drop-shadow(0 0 18px rgba(255,240,210,0.55))`,
                  ...
                  <PvElement as={"rect"} x={2} y={2} width={bw - 4} height={bh - 4} rx={RADIUS} fill="none" stroke={AMBER} strokeWidth={beam1On ? 5 : 3.5} strokeLinecap="round" pathLength={1} strokeDasharray="0.14 1" strokeDashoffset={-(beam1On ? beam1Prog : beam2Prog)}/>
                  <PvElement as={"rect"} x={2} y={2} width={bw - 4} height={bh - 4} rx={RADIUS} fill="none" stroke={BEAM_CORE} strokeWidth={beam1On ? 2.5 : 1.75} strokeLinecap="round" pathLength={1} strokeDasharray="0.14 1" strokeDashoffset={-(beam1On ? beam1Prog : beam2Prog)}/>
                </PvElement>) : null}

              {/* faint amber ring left behind, fades out */}
              {beamTrail > 0.01 ? (<PvElement as={"div"} style={{
          ...
          {frame >= 60 && frame <= 130 ? (() => {
            ...
            return (<PvElement as={"div"} style={{ transformStyle: 'preserve-3d', pointerEvents: 'none' }}>
                <PvElement as={"div"} style={{
                ...
                <PvElement as={"div"} style={{
                  ...
                  <PvElement as={"div"} style={{ fontFamily: SERIF, fontSize: 37, fontWeight: 600, color: INK, lineHeight: 1.16, letterSpacing: '-0.012em' }}>
                    One card,
                  </PvElement>
                  <PvElement as={"div"} style={{ position: 'relative', display: 'inline-block' }}>
                    <PvElement as={"div"} style={{
                    position: 'absolute', left: -5, top: '12%', bottom: '4%',
                    width: `calc(${hl} * (100% + 10px))`,
                    background: 'oklch(88% 0.095 85)', borderRadius: 4,
                }}/>
                    <PvElement as={"div"} style={{ position: 'relative', fontFamily: SERIF, fontStyle: 'italic', fontSize: 37, fontWeight: 600, color: INK, lineHeight: 1.16, letterSpacing: '-0.012em' }}>
                      one project.
                    </PvElement>
                  </PvElement>
                </PvElement>
