...
const makeTrib = (start: Pt, mergeX: number, opacity: number, growStart: number, growDur: number): Trib => {
...
const tribPath = (t: Trib) => `M ${t.p0.x} ${t.p0.y} C ${t.p1.x} ${t.p1.y}, ${t.p2.x} ${t.p2.y}, ${t.p3.x} ${t.p3.y}`;
...
const farPath = (l: FarLine) => `M ${l.p0.x} ${l.p0.y} C ${l.p1.x} ${l.p1.y}, ${l.p2.x} ${l.p2.y}, ${l.p3.x} ${l.p3.y}`;
...
export const DatavizLandscapeOpen: React.FC = () => {
    ...
    const flowIn = (gEnd: number) => interpolate(frame, [gEnd, gEnd + 20], [0, 1], {
    ...
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: '#050505', overflow: 'hidden' }}>
      {/* zoom 容器: 以画面中心为原点 */}
      <PvElement as={AbsoluteFill} style={{ transform: `scale(${zoom})`, transformOrigin: '50% 50%' }}>
        {/* 底色微渐变, 避免纯黑死平 */}
        ...
        {/* ---- 远景层 (视差 0.6×) ---- */}
        <PvElement as={"div"} style={layerStyle(farX)}>
          <PvElement as={"svg"} width={WORLD_W} height={H} style={{ position: 'absolute' }}>
            {FAR_LINES.map((l, i) => {
            const g = growth(frame, l.growStart, l.growDur);
            return (<PvElement as={"g"} key={i}>
                  <PvElement as={"path"} d={farPath(l)} fill="none" stroke="#ffffff" strokeWidth={1.3} strokeLinecap="round" opacity={l.op} pathLength={1} strokeDasharray={1} strokeDashoffset={1 - g}/>
                  {/* 节点标记: 可见特征点穿过画面, 让 0.6× 层位移可辨 */}
                  {FAR_NODE_TS.map((t, j) => {
                    const p = cubicAt(l.p0, l.p1, l.p2, l.p3, t);
                    return (<PvElement as={"circle"} key={j} cx={p.x} cy={p.y} r={3} fill="#ffffff" opacity={t <= g ? l.op + 0.08 : 0}/>);
                })}
                </PvElement>);
        })}
          </PvElement>
        ...
        {/* ---- 中景层 (主角层, 视差 1×) ---- */}
        <PvElement as={"div"} style={layerStyle(midX)}>
          <PvElement as={"svg"} width={WORLD_W} height={H} style={{ position: 'absolute' }}>
            {/* 微辉光垫层: 1–2px soft glow 压出"发光线"(卡坑注, Q4 单点许可内) */}
            <PvElement as={"g"} style={{ filter: 'blur(3px)' }}>
              <PvElement as={"path"} d={trunkPath()} fill="none" stroke="#ffffff" strokeWidth={7} strokeLinecap="round" opacity={0.16} pathLength={1} strokeDasharray={1} strokeDashoffset={1 - trunkGrow}/>
              {MID_TRIBS.map((t, i) => (<PvElement as={"path"} key={i} d={tribPath(t)} fill="none" stroke="#ffffff" strokeWidth={5.5} strokeLinecap="round" opacity={t.opacity * 0.18} pathLength={1} strokeDasharray={1} strokeDashoffset={1 - growth(frame, t.growStart, t.growDur)}/>))}
            </PvElement>
            {/* 主干 (唯一) */}
            <PvElement as={"path"} d={trunkPath()} fill="none" stroke="#ffffff" strokeWidth={2.8} strokeLinecap="round" opacity={0.8} pathLength={1} strokeDasharray={1} strokeDashoffset={1 - trunkGrow}/>
            {/* 支流, 错峰生长 */}
            {MID_TRIBS.map((t, i) => (<PvElement as={"path"} key={i} d={tribPath(t)} fill="none" stroke="#ffffff" strokeWidth={2.2} strokeLinecap="round" opacity={t.opacity} pathLength={1} strokeDasharray={1} strokeDashoffset={1 - growth(frame, t.growStart, t.growDur)}/>))}
            {/* 流动感: 虚线相位沿线漂向汇点 (1.5px/f, 叠加透明度 ≤0.3) */}
            <PvElement as={"path"} d={trunkPath()} fill="none" stroke="#ffffff" strokeWidth={2.8} strokeLinecap="round" opacity={0.3 * flowIn(38)} strokeDasharray={flowDash} strokeDashoffset={flowOffset}/>
            {MID_TRIBS.map((t, i) => (<PvElement as={"path"} key={`flow-${i}`} d={tribPath(t)} fill="none" stroke="#ffffff" strokeWidth={2.2} strokeLinecap="round" opacity={0.26 * flowIn(t.growStart + t.growDur)} strokeDasharray={flowDash} strokeDashoffset={flowOffset}/>))}
          </PvElement>
        ...
        {/* ---- 近景层 (大虚焦, 视差 1.4×) ---- */}
        <PvElement as={"div"} style={{ ...layerStyle(nearX), filter: 'blur(13px)' }}>
          <PvElement as={"svg"} width={WORLD_W} height={H} style={{ position: 'absolute' }}>
            {NEAR_LINES.map((l, i) => (<PvElement as={"path"} key={i} d={l.d} fill="none" stroke="#ffffff" strokeWidth={l.w} strokeLinecap="round" opacity={l.op} pathLength={1} strokeDasharray={1} strokeDashoffset={1 - growth(frame, l.growStart, l.growDur)}/>))}
          </PvElement>
        </PvElement>
