...
export const LetterspaceMaterialize: React.FC = () => {
    ...
    const letters = pvCopy(WORD).split('').map((ch, li) => (<PvElement as={"svg"} key={li} width={78} height={64} viewBox="0 0 78 64" style={{ overflow: 'visible', display: 'block' }}>
      {p > 0 && (<PvElement as={"path"} d={GLYPHS[ch]} fill="none" stroke="#f4f2f8" strokeWidth={5.5} strokeLinecap="round" strokeLinejoin="round" pathLength={1} strokeDasharray={1} strokeDashoffset={1 - e} style={{
    ...
    return (<PvElement as={AbsoluteFill} style={{
      ...
      {/* 大字距字标：全字符并行连续描画 */}
      <PvElement as={"div"} style={{ display: 'flex', gap: 34, alignItems: 'center' }}>
        {letters}
      </PvElement>
    </PvElement>);
};
