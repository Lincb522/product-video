...
export const StrokeSegmentBuild: React.FC = () => {
    ...
    const pulse = interpolate(frame, [LAST_LAND, LAST_LAND + 3, PULSE_END], [1, 1.06, 1], { easing: Easing.out(Easing.cubic), extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.ink, overflow: 'hidden', position: 'relative' }}>
      ...
      <PvElement as={"svg"} width={1920} height={1080} style={{ position: 'absolute', left: 0, top: 0, transform: `scale(${pulse})`, transformOrigin: '50% 55%' }}>
        {SEGS.map((seg, i) => {
            const rank = ORDER.indexOf(i);
            const start = FIRST + rank * STEP;
            if (frame < start)
                return null; // 未开始的段不渲染
            ...
            return (<PvElement as={"line"} key={i} x1={OX + seg.x1 + ox} y1={OY + seg.y1 + oy} x2={OX + seg.x2 + ox} y2={OY + seg.y2 + oy} stroke={G.panel} strokeWidth={44} strokeLinecap="butt" opacity={t}/>);
        })}
      </PvElement>
    </PvElement>);
};
