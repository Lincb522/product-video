...
export const DominoCascade: React.FC = () => {
    ...
    const lastCardRot = interpolate(frame, [IMPACT_2 - 9, IMPACT_2, SIDE_END], [0, -3, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
    // ③ 侧边栏横向撞滑：带初速滑入 + 过冲回弹
    let sideX: number;
    if (frame < IMPACT_2) {
        sideX = -260;
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, overflow: 'hidden', position: 'relative' }}>
      {/* 全画面震动容器 */}
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, transform: `translateY(${shakeY}px)` }}>
        {/* 地板线：卡片落点 */}
        <PvElement as={"div"} style={{ position: 'absolute', left: ROW_LEFT - 30, top: 928, width: ROW_W + 60, height: 6, background: G.bar, borderRadius: 3 }}/>

        {/* ① 砸落的标题 */}
        <PvElement as={"div"} style={{ position: 'absolute', left: 240, width: 1680, top: titleTop, display: 'flex', justifyContent: 'center' }}>
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"div"} style={{ width: 44, height: 44, borderRadius: 10, background: '#777775' }}/>
          {Array.from(pvArrayCopy({ length: 8 })).map((_, i) => (<PvElement as={"div"} key={i} style={{ height: 13, width: `${58 + ((i * 31) % 38)}%`, background: G.sideBar, borderRadius: 6 }}/>))}
        </PvElement>
      </PvElement>
    </PvElement>);
};
