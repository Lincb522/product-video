...
const CloseupStage: React.FC<{
    children: React.ReactNode;
}> = ({ children }) => (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.panel, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
export const WrightTripleCut: React.FC = () => {
    ...
    if (f < C2) {
        ...
        return (<CloseupStage>
        <PvElement as={"div"} style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <PvElement as={"div"} style={{
            ...
            <PvElement as={"div"} style={{ width: 260, height: 30, borderRadius: 15, background: p < 0.5 ? G.card : G.line }}/>
          </PvElement>
          {/* 圆形光标点:悬停在按钮中心,按下缩一圈 */}
          <PvElement as={"div"} style={{
    ...
    if (f < WHIP) {
        ...
        return (<CloseupStage>
        <PvElement as={"div"} style={{ perspective: 1200 }}>
          <PvElement as={"div"} style={{
    ...
    const dash = (x: number, opacity: number, key: string) => (<PvElement as={"div"} key={key} style={{ position: 'absolute', left: 0, top: 0, transform: `translateX(${x}px)`, opacity }}>
