...
const Footage: React.FC = () => (<PvElement as={"div"} style={{ position: 'absolute', width: 1920, height: 1080 }}>
    <FakeDashboard variant="A"/>
    <PvElement as={"div"} style={{ position: 'absolute', left: 1070, top: 350, transform: 'translate(-50%, -50%)' }}>
...
export const PaparazziFlash: React.FC = () => {
    ...
    for (const f of FLASHES) {
        ...
        if (frame >= f && frame < f + DECAY + 1)
            flash = Math.max(flash, o);
    }
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: G.ink, overflow: 'hidden' }}>
      ...
      {/* 白闪层 */}
      {flash > 0 && (<PvElement as={AbsoluteFill} style={{ background: '#ffffff', opacity: flash }}/>)}
    </PvElement>);
};
