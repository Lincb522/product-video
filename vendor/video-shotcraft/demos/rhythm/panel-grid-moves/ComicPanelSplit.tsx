...
const PageA: React.FC = () => (<PvElement as={"div"} style={{ width: 1920, height: 1080, position: 'relative' }}>
    ...
    <PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{ fontFamily: 'Helvetica, Arial, sans-serif', fontWeight: 800, fontSize: 96, color: G.ink, letterSpacing: -2, lineHeight: 1 }}>
        1,284
      </PvElement>
      <PvElement as={"div"} style={{ height: 10, width: 150, background: G.mid, borderRadius: 5 }}/>
    </PvElement>
  </PvElement>);
type PanelSpec = {
...
export const ComicPanelSplit: React.FC = () => {
    ...
    // ===== 阶段 1：全屏 A =====
    if (frame < SPLIT) {
        return (<PvElement as={AbsoluteFill} style={{ background: G.bg }}>
        <PageA />
      </PvElement>);
    }
    // ===== 阶段 2/3：分格 + 第三格扩张 =====
    ...
    const seam1O = Math.min(interpolate(frame, [SPLIT + STAGGER, SPLIT + STAGGER + 2], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }), interpolate(frame, [HOLD_END, HOLD_END + 3], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }));
    const seam2O = Math.min(interpolate(frame, [SPLIT + 2 * STAGGER, SPLIT + 2 * STAGGER + 2], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }), interpolate(frame, [EXPAND_END - 4, EXPAND_END], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }));
    return (<PvElement as={AbsoluteFill} style={{ background: '#ffffff' }}>
      {panels.map((p, i) => {
            ...
            {pulse > 0.005 && (<PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: `rgba(0,0,0,${pulse})` }}/>)}
          </PvElement>);
        })}
      {/* 斜缝：白缝命门 + 墨线描边（暗 16 下、白 10 上 → 两侧各 3px 墨边） */}
      {(seam1O > 0.005 || seam2O > 0.005) && (<PvElement as={"svg"} width={1920} height={1080} style={{ position: 'absolute', inset: 0, zIndex: 5, pointerEvents: 'none' }}>
          {seam1O > 0.005 && (<PvElement as={"g"} opacity={seam1O}>
              <PvElement as={"line"} x1={750} y1={-10} x2={520} y2={1090} stroke="#2f2f2f" strokeWidth={16}/>
              <PvElement as={"line"} x1={750} y1={-10} x2={520} y2={1090} stroke="#ffffff" strokeWidth={10}/>
            </PvElement>)}
          {seam2O > 0.005 && (<PvElement as={"g"} opacity={seam2O}>
              <PvElement as={"line"} x1={e3Top - 5} y1={-10} x2={e3Bot - 5} y2={1090} stroke="#2f2f2f" strokeWidth={16}/>
              <PvElement as={"line"} x1={e3Top - 5} y1={-10} x2={e3Bot - 5} y2={1090} stroke="#ffffff" strokeWidth={10}/>
            </PvElement>)}
        </PvElement>)}
    </PvElement>);
};
