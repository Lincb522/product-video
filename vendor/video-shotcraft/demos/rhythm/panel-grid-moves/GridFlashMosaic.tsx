...
const CellContent: React.FC<{
    ...
    return (<PvElement as={"div"} style={{ position: 'absolute', left: crop.x, top: crop.y }}>
...
export const GridFlashMosaic: React.FC = () => {
    ...
    const breath = f >= LAST_IN && f < HOLD_END
    ...
    return (<PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{
        ...
        {Array.from(pvArrayCopy({ length: 9 })).map((_, i) => {
            const start = FILL_START + RANK[i] * STEP;
            if (f < start)
                return null; // 硬入：未到拍点不渲染，无淡化
            ...
            return (<PvElement as={"div"} key={i} style={{
              ...
              <CellContent i={i}/>
              {darken > 0.001 && (<PvElement as={"div"} style={{
                        position: 'absolute',
                        inset: 0,
                        background: '#000',
                        opacity: darken,
