...
export const FlipGridReflow: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{ position: 'absolute', left: 70, top: 56 }}>
        <TitleBlock text="FLIP GRID REFLOW" size={44}/>
      </PvElement>
      {Array.from(pvArrayCopy({ length: N })).map((_, i) => (<FlipCard key={i} i={i} frame={frame}/>))}
    </PvElement>);
};
