...
const posAt = (f: number): number => f < 37
...
export const SmearMultiples: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      ...
      {ghostOps.map((baseOp, i) => {
            ...
            if (op <= 0.001)
                return null;
            return (<PvElement as={"div"} key={k} style={{ position: 'absolute', left: gx, top: Y, opacity: op }}>
            <Card w={480} h={320} seed={5}/>
          </PvElement>);
        })}
      <PvElement as={"div"} style={{ position: 'absolute', left: bodyX, top: Y }}>
        <Card w={480} h={320} seed={5}/>
      </PvElement>
    </PvElement>);
};
