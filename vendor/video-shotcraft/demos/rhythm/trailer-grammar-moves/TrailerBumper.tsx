...
const push = (frame: number, start: number) => interpolate(frame, [start, start + 9], [1, 1.04], {
...
export const TrailerBumper: React.FC = () => {
    ...
    if (frame < BLACK) {
        ...
        if (frame < CUT_2) {
            // 镜头1：A 整页 + 轻推近
            inner = (<PvElement as={"div"} style={{ width: 1920, height: 1080, transform: `scale(${push(frame, CUT_1)})`, transformOrigin: '50% 50%' }}>
          <FakeDashboard variant="A"/>
        </PvElement>);
        }
        else if (frame < CUT_3) {
            // 镜头2：B 整页 + 轻推近
            inner = (<PvElement as={"div"} style={{ width: 1920, height: 1080, transform: `scale(${push(frame, CUT_2)})`, transformOrigin: '50% 50%' }}>
          <FakeDashboard variant="B"/>
            ...
            inner = (<PvElement as={"div"} style={{ width: 1920, height: 1080, transform: `scale(${s})`, transformOrigin: '58% 40%' }}>
          <FakeDashboard variant="A"/>
        </PvElement>);
        }
        return (<PvElement as={"div"} style={{ width: 1920, height: 1080, overflow: 'hidden', background: G.bg }}>
        {inner}
      </PvElement>);
    }
    // —— 段落 4：纯黑静默 6f，必须纯黑无物 ——
    if (frame < TITLE) {
        return <PvElement as={"div"} style={{ width: 1920, height: 1080, background: '#000000' }}/>;
    }
