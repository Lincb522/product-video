...
const TitleCard: React.FC<{
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: '#0d0d0d', alignItems: 'center', justifyContent: 'center' }}>
      <PvElement as={"div"} style={{
        ...
        {underline && (<PvElement as={"div"} style={{ width: 560, height: 14, background: '#ffffff', borderRadius: 7 }}/>)}
      </PvElement>
    </PvElement>);
};
...
const UiShot: React.FC<{
    ...
    transform: string;
    origin?: string;
}> = ({ variant, transform, origin = '50% 50%' }) => (<PvElement as={AbsoluteFill} style={{ overflow: 'hidden', background: '#ececea' }}>
    <PvElement as={"div"} style={{ width: 1920, height: 1080, transform, transformOrigin: origin }}>
...
export const CardFootageCadence: React.FC = () => {
    ...
    if (frame < 34) {
        ...
        return (<UiShot variant="B" transform={`scale(1.6) translateX(${tx}px)`} origin="35% 50%"/>);
    }
    // 段4 34–42f：字卡 FASTER
    if (frame < 42) {
