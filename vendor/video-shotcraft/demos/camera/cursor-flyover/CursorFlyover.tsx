...
const acc = (t: number, base: Vec, kfs: {
    at: number[];
    to: Vec;
}[], keys: string[], ease: (x: number) => number) => {
    const out: Vec = {};
    for (const k of keys)
        out[k] = base[k];
    let prev = base;
...
const LINE_D = (() => {
    let dd = '';
    for (let i = 0; i <= 14; i++)
        dd += `${i === 0 ? 'M' : 'L'}${i * 7.14},${40 - (6 + rand(i * 5) * 22 + i * 0.7)} `;
    return dd;
})();
// 四象限容器（截图内相对定位）
const Quad: React.FC<{
...
}> = ({ x, y, w, h, children, }) => (<PvElement as={"div"} style={{
        position: 'absolute',
        left: `${x}%`,
        top: `${y}%`,
        width: `${w}%`,
...
export const CursorFlyover: React.FC = () => {
    ...
    return (<DesignStage bg="#080910" raster="zoom">
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: '#080910', overflow: 'hidden' }}>
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"div"} style={{
            ...
            <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, width: '100%', height: '9%', background: '#1a1d28', borderBottom: '1px solid #262c3b', boxSizing: 'content-box' }}>
              {['#ff6058', '#ffbd2e', '#28ca42'].map((c, i) => (<PvElement as={"div"} key={c} style={{
            ...
            <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: '9%', width: '15%', height: '91%', background: '#161923', borderRight: '1px solid #232937', boxSizing: 'content-box' }}>
              {['Overview', 'Traffic', 'Revenue', 'Cohorts', 'Alerts'].map((s, i) => (<PvElement as={"div"} key={s} style={{
            ...
            <Quad x={18} y={14} w={38} h={36}>
              {['12.4k', '+38%', '4.1s'].map((s, i) => (<PvElement as={"div"} key={s} style={{
                ...
                background: '#202634', padding: '8px 0 0 8px', boxSizing: 'border-box',
                  ...
                  <PvElement as={"div"} style={{ font: '700 13px/1 -apple-system,sans-serif', color: '#e7ecfb' }}>{s}</PvElement>
                  <PvElement as={"div"} style={{ marginTop: 4, font: '600 6px/1 -apple-system,sans-serif', letterSpacing: 1, color: '#5d6580' }}>
                    METRIC {i + 1}
                  </PvElement>
                </PvElement>))}
            </Quad>
            ...
            <Quad x={59} y={14} w={37} h={36}>
              <PvElement as={"svg"} viewBox="0 0 100 46" style={{ position: 'absolute', left: '6%', top: '16%', width: '88%', height: '70%' }}>
                <PvElement as={"path"} d={LINE_D} fill="none" stroke="#6c8cff" strokeWidth={1.6} strokeLinejoin="round"/>
              </PvElement>
            </Quad>
            {/* 右下：柱状图 */}
            <Quad x={59} y={55} w={37} h={34}>
              {Array.from(pvArrayCopy({ length: 9 }), (_, i) => (<PvElement as={"div"} key={i} style={{
            ...
            <Quad x={18} y={55} w={38} h={34}>
              {Array.from(pvArrayCopy({ length: 5 }), (_, i) => (<PvElement as={"div"} key={i} style={{
          ...
          <PvElement as={"svg"} viewBox="0 0 24 24" style={{
            ...
            <PvElement as={"path"} d="M4 2 L4 19 L9 14.4 L12.2 21.5 L15.4 20 L12.2 13 L19 12.6 Z" fill={click > 0 && click < 0.4 ? '#dfe6ff' : '#fff'} stroke="#101320" strokeWidth={1.1}/>
          </PvElement>
        </PvElement>
      </PvElement>
    </DesignStage>);
