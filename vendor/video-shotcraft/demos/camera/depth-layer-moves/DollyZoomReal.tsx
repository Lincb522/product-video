...
export const DollyZoomReal: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: '#efece6', overflow: 'hidden' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={Img} src={staticFile('textures/live/projects-full.png')} style={{ position: 'absolute', left: 0, top: -400, width: 1920, opacity: 0.55 }}/>
        {BG_CARDS.slice(0, 8).map((c, k) => (<PvElement as={Img} key={c.file + k} src={staticFile(`textures/live/${c.file}`)} style={{
                position: 'absolute',
