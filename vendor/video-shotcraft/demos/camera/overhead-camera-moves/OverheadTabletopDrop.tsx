...
const PageCard: React.FC<{
...
}> = ({ x, children }) => (<PvElement as={"div"} style={{
        position: 'absolute',
        left: x - CARD_W / 2,
        top: -CARD_H / 2,
        width: CARD_W,
    ...
    <PvElement as={"div"} style={{ width: 1920, height: 1080, transform: `scale(${INNER})`, transformOrigin: '0 0' }}>
      {children}
    </PvElement>
  </PvElement>);
export const OverheadTabletopDrop: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: G.bg, overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, perspective: 1600, perspectiveOrigin: '50% 42%' }}>
