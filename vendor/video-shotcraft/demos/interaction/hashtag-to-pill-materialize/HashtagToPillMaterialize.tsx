...
const NoteIcon: React.FC<{
    size: number;
    color: string;
}> = ({ size, color }) => (<PvElement as={"svg"} width={size} height={size} viewBox="0 0 48 48" style={{ display: 'block' }}>
    <PvElement as={"ellipse"} cx="11" cy="39" rx="7.2" ry="5.6" fill={color} transform="rotate(-18 11 39)"/>
    <PvElement as={"ellipse"} cx="35" cy="35" rx="7.2" ry="5.6" fill={color} transform="rotate(-18 35 35)"/>
    <PvElement as={"rect"} x="15.4" y="12.5" width="3" height="27" fill={color}/>
    <PvElement as={"rect"} x="39.4" y="8.5" width="3" height="27" fill={color}/>
    <PvElement as={"polygon"} points="15.4,12.5 42.4,8.5 42.4,16.5 15.4,20.5" fill={color}/>
  </PvElement>);
...
const Pill: React.FC<{
    ...
    iconColor: string;
    textColor: string;
}> = ({ bg, iconColor, textColor }) => (<PvElement as={"div"} style={{
    ...
    <PvElement as={"span"} style={{ fontSize: FS, fontWeight: 500, color: textColor, letterSpacing: 2 }}>music</PvElement>
  </PvElement>);
export const HashtagToPillMaterialize: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: revealed ? C.bgCream : C.bgWhite, fontFamily: FONT }}>
      {/* 成品页（硬切揭示，之后全静） */}
      {revealed && (<>
          ...
          <PvElement as={"div"} style={{
            ...
            I want to share a few of my favorite bands<PvElement as={"br"}/>
            and the song that I always listen when driving<PvElement as={"br"}/>
            to home. Welcome. Bring headphones.
          </PvElement>
        </>)}

      {/* 打字层：文字 + 恒亮红光标（原片光标不闪烁），实体化帧整体消失 */}
      {frame < MORPH && (<PvElement as={AbsoluteFill} style={{ justifyContent: 'center', alignItems: 'center' }}>
          <PvElement as={"div"} style={{ display: 'flex', alignItems: 'center' }}>
            <PvElement as={"span"} style={{ fontSize: FS, fontWeight: 500, color: C.ink, letterSpacing: 2, whiteSpace: 'pre' }}>
              {typed}
            </PvElement>
      ...
      {frame >= MORPH && (<PvElement as={"div"} style={{
          ...
          <PvElement as={"div"} style={{ transform: 'translate(-50%, -50%)' }}>
            {revealed
