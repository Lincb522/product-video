...
export const GlassPillDictationTyping: React.FC = () => {
    ...
    useLayoutEffect(() => {
        if (measRef.current)
            setTextW(measRef.current.offsetWidth);
    }, []);
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"div"} style={{
            ...
            {/* 声波竖条轻微呼吸（原片几乎静止，仅微动） */}
            <PvElement as={"div"} style={{ display: 'flex', alignItems: 'center', gap: 2.2, height: '100%' }}>
              {BASE.map((h, i) => (<PvElement as={"div"} key={i} style={{
                width: 1.5,
                borderRadius: 1.5,
                background: '#ececee',
        ...
        <PvElement as={"div"} ref={measRef} style={{
            position: 'absolute',
            visibility: 'hidden',
            whiteSpace: 'pre',
            font: `400 21px ${UI}`,
