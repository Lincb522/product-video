...
const PaletteRow: React.FC<{
    ...
    return (<PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ width: 36, height: 36, borderRadius: 9, background: G.mid }}/>
        <PvElement as={"div"} style={{ height: 14, width: `${titleW}%`, background: G.bar, borderRadius: 7 }}/>
        <PvElement as={"div"} style={{ marginLeft: 'auto', width: 58, height: 24, borderRadius: 6, background: G.line }}/>
      </PvElement>
    </PvElement>);
};
export const CommandPaletteSummon: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ filter: frame < DIM0 ? undefined : `blur(${blur}px)` }}>
        <FakeDashboard variant="A"/>
      </PvElement>
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: `rgba(20,20,20,${dim})` }}/>

      {frame >= PANEL_IN && (<PvElement as={"div"} style={{
                position: 'absolute',
                left: PANEL_X,
                top: PANEL_Y,
                width: PANEL_W,
          ...
          <PvElement as={"div"} style={{
            ...
            <PvElement as={"div"} style={{ width: 32, height: 32, borderRadius: 8, background: G.bar }}/>
            {/* 已敲入的字符灰块 */}
            {Array.from(pvArrayCopy({ length: typed })).map((_, c) => (<PvElement as={"div"} key={c} style={{ width: 28, height: 38, borderRadius: 6, background: '#4a4a48' }}/>))}
            {/* 光标 */}
            {cursorOn && <PvElement as={"div"} style={{ width: 4, height: 42, background: G.ink, borderRadius: 2 }}/>}
            {/* 占位提示条：敲第一个字母时卸载 */}
            {typed === 0 && (<PvElement as={"div"} style={{ width: 260, height: 14, borderRadius: 7, background: G.line, opacity: 0.8 }}/>)}
          </PvElement>
          {/* 候选行 */}
          <PvElement as={"div"} style={{ paddingTop: 14 }}>
            {ROWS.map((_, i) => (<PaletteRow key={i} i={i} frame={frame}/>))}
          </PvElement>
        </PvElement>)}
    </PvElement>);
};
