...
const Cursor: React.FC<{
    x: number;
    y: number;
}> = ({ x, y }) => (<PvElement as={"svg"} width={48} height={48} viewBox="0 0 28 28" style={{
        position: 'absolute',
        left: x - 3.4,
        top: y - 1.7,
        filter: 'drop-shadow(0 4px 7px rgba(0,0,0,0.4))',
    }}>
    <PvElement as={"path"} d="M2 1 L2 23 L8 17.5 L11.5 25 L15.5 23.2 L12 15.8 L20 15 Z" fill="#ffffff" stroke="#2f2f2f" strokeWidth={1.6} strokeLinejoin="round"/>
  </PvElement>);
export const CursorPerformancePunchIn: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, overflow: 'hidden', position: 'relative' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        {/* 涟漪圆环：条件挂载，f66 后摘除 */}
        {rippleAlive && (<PvElement as={"div"} style={{
                position: 'absolute',
