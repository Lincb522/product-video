...
const Keycap: React.FC<{
    ...
    bright: number;
    label?: string;
}> = ({ size, thick, bright, label = '⌘⏎', }) => (<PvElement as={"div"} style={{
...
const passWindow = (i: number, k: number): [
    number,
    number
] => {
...
const FlyPanel: React.FC<{
    ...
    <PvElement as={"div"} style={{ height: 52, border: `2px solid ${G.line}`, borderRadius: 12, background: G.panel }}/>
    {Array.from(pvArrayCopy({ length: 3 })).map((_, i) => (<PvElement as={"div"} key={i} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 16 }}>
        <PvElement as={"div"} style={{ width: 28, height: 28, borderRadius: 8, background: G.mid }}/>
        <PvElement as={"div"} style={{ height: 12, width: `${50 + ((i * 27) % 30)}%`, background: G.line, borderRadius: 6 }}/>
      </PvElement>))}
  </PvElement>);
};
const FlyItem: React.FC<{
    fly: Fly;
    i: number;
    frame: number;
}> = ({ fly, i, frame }) => {
    ...
    return (<PvElement as={"div"} style={{
      ...
      {fly.panel
            ...
            : <Card w={fly.w} h={fly.h} seed={fly.seed} style={{ boxShadow: '0 12px 40px rgba(0,0,0,0.25)' }}/>}
    </PvElement>);
};
export const KeycapSmashCut: React.FC = () => {
    ...
    //    无任何随帧变化的属性 ——
    if (frame >= T.cut) {
        return (<PvElement as={"div"} style={{ width: 1920, height: 1080, position: 'relative' }}>
        <FakeDashboard variant="A"/>
        {/* 键帽句号：稳稳嵌在顶栏（标题条右侧空档），呼应引信 */}
        <PvElement as={"div"} style={{ position: 'absolute', left: 464, top: 12 }}>
    ...
    return (<PvElement as={"div"} style={{
      ...
      {/* 轰鸣段飞入物 */}
      {frame >= T.pressEnd && FLIES.map((fly, i) => (<FlyItem key={i} fly={fly} i={i} frame={frame}/>))}
    </PvElement>);
};
