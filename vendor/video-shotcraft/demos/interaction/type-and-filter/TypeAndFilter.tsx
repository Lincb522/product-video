...
cards.forEach((_, i) => {
    if (i !== nanoIdx)
        leaveRank.set(i, leaveRank.size);
});
...
export const TypeAndFilter: React.FC = () => {
    ...
    const caretOn = frame >= TYPE_START - 2 &&
        frame <= 67 &&
        (frame <= TYPE_START + 24 || Math.floor((frame - (TYPE_START + 24)) / 8) % 2 === 0);
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: PAPER }}>
      <PageCam2D src="textures/live/projects-empty.png" pageH={PAGE_H} keys={CAM_KEYS}>
        ...
        {cards.map((c, i) => {
            ...
            return (<PvElement as={"div"} key={`${c.file}-${i}`} style={{
              ...
              <PvElement as={Img} src={staticFile(`textures/live/${c.file}`)} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }}/>
            </PvElement>);
        })}
        ...
        {(() => {
            ...
            return (<PvElement as={"div"} style={{
              ...
              <PvElement as={Img} src={staticFile(`textures/live/${nano.file}`)} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }}/>
            </PvElement>);
        })()}

        {/* ---- search box: paper patch over the placeholder (keeps magnifier), typed text + caret ---- */}
        {frame >= 0 ? (<PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"span"}>{QUERY.slice(0, typedCount)}</PvElement>
          {caretOn ? (<PvElement as={"span"} style={{ display: 'inline-block', width: 2, height: 20, marginLeft: 2, background: AMBER }}/>) : null}
        </PvElement>

        {/* ---- click ripple: two concentric amber rings ---- */}
        {[0, 1].map((r) => {
            const start = 58 + r * 3; // 176 - 118
            if (frame < start || frame > start + 10)
                return null;
            const t = interpolate(frame, [start, start + 10], [0, 1], {
                extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.out(Easing.cubic),
            });
            const rad = interpolate(t, [0, 1], [14, r === 0 ? 54 : 78]);
            return (<PvElement as={"div"} key={`ripple-${r}`} style={{
        ...
        {frame >= 60 ? (<PvElement as={"div"} style={{
