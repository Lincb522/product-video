...
export const DiagramCascadeBuild: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: G.bg, overflow: 'hidden' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ width: 30, height: 30, borderRadius: 15, background: G.mid, marginRight: 20, flexShrink: 0 }}/>
        <PvElement as={"span"}>
          {PROMPT.slice(0, typed)}
          <PvElement as={"span"} style={{ opacity: caretOn ? 1 : 0, fontWeight: 400 }}>|</PvElement>
        </PvElement>
      </PvElement>

      {/* 树（呼吸缩放以树心为原点） */}
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, transform: `scale(${breathe})`, transformOrigin: '960px 620px' }}>
        {/* 连线层 */}
        <PvElement as={"svg"} width={1920} height={1080} style={{ position: 'absolute', inset: 0 }}>
          {NODES.filter((n) => n.parent >= 0).map((n) => {
            ...
            if (grow <= 0)
                return null;
            return (<PvElement as={"path"} key={n.id} d={edgePath(p, n)} stroke={G.mid} strokeWidth={5} fill="none" strokeLinejoin="round" strokeDasharray={len} strokeDashoffset={len * (1 - grow)}/>);
        })}
        </PvElement>
        ...
        {NODES.map((n) => {
            ...
            if (frame < start)
                return null;
            return (<PvElement as={"div"} key={n.id} style={{
                    position: 'absolute',
                    left: n.x - NODE_W / 2,
                    top: n.y - NODE_H / 2,
                    width: NODE_W,
              ...
              <PvElement as={"div"} style={{ height: 16, width: `${44 + (n.id * 17) % 34}%`, background: n.level === 0 ? '#9a9a98' : G.bar, borderRadius: 8 }}/>
              <PvElement as={"div"} style={{ height: 10, width: `${70 - (n.id * 13) % 26}%`, background: n.level === 0 ? G.sideBar : G.line, borderRadius: 5 }}/>
            </PvElement>);
        })}
      </PvElement>
    </PvElement>);
