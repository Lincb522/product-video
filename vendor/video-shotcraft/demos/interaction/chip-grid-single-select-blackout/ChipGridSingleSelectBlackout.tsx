...
export const ChipGridSingleSelectBlackout: React.FC = () => {
    ...
    useLayoutEffect(() => {
        ...
        if (el && el.offsetWidth)
            setCx(220 - (el.offsetLeft + el.offsetWidth / 2));
    }, []);
    ...
    return (<DesignStage bg={BG}>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ position: 'absolute', left: '50%', top: '50%', width: 440, height: 240, margin: '-120px 0 0 -220px' }}>
          ...
          {[0, 1].map((row) => (<PvElement as={"div"} key={row} style={{
                position: 'absolute',
                left: 0,
                right: 0,
                top: row === 0 ? 96 : 136,
              ...
              {NAMES.map((n, i) => {
                ...
                return (<PvElement as={"div"} key={i} ref={i === TI ? targetRef : undefined} style={{
                        position: 'relative',
                        height: 30,
                        borderRadius: 15,
                        background: bg,
                    ...
                    <PvElement as={"div"} style={{ font: `600 12px/1 ${SANS}`, color: labelColor, letterSpacing: '-.01em', whiteSpace: 'nowrap' }}>
                      {n}
                    </PvElement>
                    {/* 按压灰闪块（盖在 label 上） */}
                    <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: 'rgba(120,120,120,.5)', opacity: flashO }}/>
                  </PvElement>);
            })}
            </PvElement>))}
