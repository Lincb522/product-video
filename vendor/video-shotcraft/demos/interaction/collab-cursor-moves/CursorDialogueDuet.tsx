...
const Cursor: React.FC<{
    ...
    badgeOpacity?: number;
}> = ({ x, y, scale, color, name, badgeLit, badgeOpacity = 1 }) => (<PvElement as={"div"} style={{ position: 'absolute', left: x, top: y, transform: `scale(${scale})`, transformOrigin: '0 0' }}>
    <PvElement as={"svg"} width={42} height={62} viewBox="0 0 13.5 20" style={{ display: 'block', overflow: 'visible' }}>
      <PvElement as={"path"} d="M0.5 0.5 L0.5 17.2 L4.7 13.4 L7.3 19.5 L10 18.3 L7.4 12.3 L13 12.3 Z" fill={color} stroke="#ffffff" strokeWidth={1.1} strokeLinejoin="round"/>
    </PvElement>
...
export const CursorDialogueDuet: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: DARK, position: 'relative', overflow: 'hidden' }}>
      {/* 暗场微光晕，给纯黑一点空间感 */}
      <PvElement as={"div"} style={{
            position: 'absolute', inset: 0,
            background: `radial-gradient(ellipse 1100px 700px at 50% 48%, rgba(255,255,255,0.045), transparent 70%)`,
        }}/>
      {f <= 132 && (<Cursor x={dx} y={dy} scale={2.8 * dPulse} color={BLUE} name="Designer" badgeLit={dLit} badgeOpacity={dBadgeIn * (f > 104 ? 1 - easeInCubic(clamp01((f - 104) / 18)) : 1)}/>)}
      <Cursor x={gx} y={gy} scale={gScale} color={GREEN} name="Developer" badgeLit={gLit} badgeOpacity={gBadgeIn * (1 - clamp01((f - 102) / 12))}/>
    </PvElement>);
};
