...
const CursorActor: React.FC<{
    ...
    badge: number;
    scale?: number;
}> = ({ x, y, a, badge, scale = 2.1, }) => (<PvElement as={"div"} style={{ position: 'absolute', left: x, top: y, transform: `scale(${scale})`, transformOrigin: '0 0', zIndex: 10 }}>
    <PvElement as={"svg"} width={30} height={44} viewBox="0 0 13.5 20" style={{ display: 'block', overflow: 'visible' }}>
      <PvElement as={"path"} d="M0.5 0.5 L0.5 17.2 L4.7 13.4 L7.3 19.5 L10 18.3 L7.4 12.3 L13 12.3 Z" fill={a.color} stroke="#ffffff" strokeWidth={1.1} strokeLinejoin="round"/>
    </PvElement>
...
export const CursorCastEnsemble: React.FC = () => {
    ...
    // 便签的固定随机旋转
    const noteRots = Array.from(pvArrayCopy({ length: 4 })).map(() => (rnd() - 0.5) * 8);
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      ...
      {[
        ...
        ].map((n, i) => (<PvElement as={"div"} key={i} style={{
          ...
          <PvElement as={"div"} style={{ height: 12, width: '70%', background: G.bar, borderRadius: 6 }}/>
          <PvElement as={"div"} style={{ height: 10, width: '86%', background: G.line, borderRadius: 5 }}/>
          <PvElement as={"div"} style={{ height: 10, width: '58%', background: G.line, borderRadius: 5 }}/>
        </PvElement>))}
      {/* 中央大便签：Rita 打字的舞台 */}
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ height: 14, width: 180, background: G.bar, borderRadius: 7, marginBottom: 24 }}/>
        <PvElement as={"div"} style={{
