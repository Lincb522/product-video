...
const MarqueeRow: React.FC<{
    ...
    return (<PvElement as={"div"} style={{
      ...
      {Array.from(pvArrayCopy({ length: copies })).map((_, i) => (<PvElement as={"span"} key={i} style={{ position: 'absolute', left: i * unitW, top: 0, whiteSpace: 'nowrap' }}>
          {word}
          <PvElement as={"span"} style={{ display: 'inline-block', transform: `translateX(${fontSize * 0.4}px)` }}>
            {'•'}
          </PvElement>
        </PvElement>))}
    </PvElement>);
...
export const NeonTripleMarquee: React.FC = () => {
    ...
    const pulse = (idx: number) => {
        ...
        if (t < 1 / 3)
            return 0.5 - 0.5 * Math.cos(t * 3 * Math.PI * 2);
        return 0;
    };
    // 入场淡入 + 结尾整组淡出
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: '#050308', overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ opacity: groupOpacity, position: 'absolute', inset: 0 }}>
        {rows.map((r, i) => (<MarqueeRow key={r.word} word={r.word} color={r.color} dir={r.dir} speed={r.speed} frame={f} y={40 + i * 350} fontSize={300} brightness={pulse(i)}/>))}
      </PvElement>
    </PvElement>);
};
