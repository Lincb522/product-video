...
const LogoLockup: React.FC<{
    opacity: number;
    scale: number;
}> = ({ opacity, scale }) => (<PvElement as={"div"} style={{
    ...
    <PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{
        ...
        {/* 方块内一个黑色小标记，避免纯白块太空 */}
        <PvElement as={"div"} style={{ width: 44, height: 44, borderRadius: 12, background: '#000' }}/>
      </PvElement>
      <PvElement as={"div"} style={{
...
export const LogoStingButton: React.FC = () => {
    ...
    if (frame < T.shotAEnd) {
        ...
        return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: '#000', position: 'relative', overflow: 'hidden' }}>
        <FakeDashboard variant="B"/>
        <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: '#000', opacity: dark }}/>
      </PvElement>);
    }
    // —— 段 2：黑场 6f ——
    if (frame < T.blackEnd) {
        return <PvElement as={"div"} style={{ width: 1920, height: 1080, background: '#000' }}/>;
    }
    ...
    if (frame >= T.holdEnd && frame < T.eggEnd) {
        ...
        return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: '#ececea', position: 'relative', overflow: 'hidden' }}>
        <PvElement as={"div"} style={{
        ...
        {tickOn && (<PvElement as={"div"} style={{
                    position: 'absolute', right: 90, bottom: 80,
                    width: 56, height: 56, borderRadius: 28, background: '#2f2f2f',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
            <PvElement as={"div"} style={{ width: 20, height: 20, borderRadius: 10, background: '#fff' }}/>
          </PvElement>)}
      </PvElement>);
    }
