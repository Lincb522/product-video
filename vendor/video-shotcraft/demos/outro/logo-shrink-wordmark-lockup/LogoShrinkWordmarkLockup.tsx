...
const Arcs: React.FC<{
    ...
    opacity: number;
    stroke?: string;
}> = ({ a0, a1, col, w, blur, opacity, stroke, }) => (<PvElement as={"g"} opacity={opacity}>
    {[
        [a0, a1],
        [a0 + 180, a1 + 180],
    ].map(([b0, b1], i) => (<PvElement as={"path"} key={i} d={arcPath(15, 15, 10.5, b0, b1)} fill="none" stroke={stroke ?? col} strokeWidth={w} strokeLinecap="round" style={blur ? { filter: `blur(${blur}px)` } : undefined}/>))}
  </PvElement>);
export const LogoShrinkWordmarkLockup: React.FC = () => {
    ...
    return (<DesignStage bg="#030509">
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"svg"} viewBox="0 0 30 30" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', overflow: 'visible' }}>
            {/* 霓虹晕（带缺口） */}
            ...
            {/* 愈合后的实心白 O */}
            <PvElement as={"circle"} cx={15} cy={15} r={10.5} fill="none" stroke="#fff" strokeWidth={5.5} opacity={heal}/>
          </PvElement>
        </PvElement>
