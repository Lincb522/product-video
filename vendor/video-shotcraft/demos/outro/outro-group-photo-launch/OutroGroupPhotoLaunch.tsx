...
export const OutroGroupPhotoLaunch: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ opacity: fadeOut }}>
      ...
      <PvElement as={AbsoluteFill} style={{
        ...
        <PvElement as={AbsoluteFill} style={{ background: 'radial-gradient(1200px 800px at 50% 48%, rgba(250,247,242,0.82), rgba(250,247,242,0.55) 60%, rgba(250,247,242,0.35))', pointerEvents: 'none' }}/>

        {/* group photo: elements fly in from all sides and settle around the wordmark */}
        <PvElement as={AbsoluteFill} style={{ pointerEvents: 'none' }}>
          {ELS.map((el) => {
            if (frame < el.cue)
                return null;
            ...
            return (<PvElement as={"div"} key={el.key}>
                {showGhost ? (<PvElement as={"div"} style={{
                    ...
                    {el.wbrCrop ? null : (<PvElement as={Img} src={staticFile(`textures/live/${el.file}`)} style={{ position: 'absolute', inset: 0, width: el.w, height: el.h, display: 'block' }}/>)}
                  </PvElement>) : null}
                ...
                {showGlow ? (<PvElement as={"div"} style={{
      ...
      {/* opening light sweep */}
      {sweepOpacity > 0 ? (<PvElement as={AbsoluteFill} style={{ pointerEvents: 'none', mixBlendMode: 'overlay' }}>
          <PvElement as={"div"} style={{
      ...
      {/* stage light behind the wordmark */}
      {stageLight > 0 ? (<PvElement as={AbsoluteFill} style={{
                pointerEvents: 'none',
      ...
      {/* closing vignette */}
      {vignette > 0 ? (<PvElement as={AbsoluteFill} style={{
                pointerEvents: 'none',
      ...
      {/* wordmark sign-off */}
      <PvElement as={AbsoluteFill} style={{ justifyContent: 'center', alignItems: 'center', pointerEvents: 'none' }}>
        <PvElement as={"div"} style={{ textAlign: 'center' }}>
          <PvElement as={"div"} style={{ fontFamily: SERIF, fontSize: 148, fontWeight: 600, color: 'oklch(18% 0.006 82)', letterSpacing: `${wordSpacing}em`, display: 'flex' }}>
            {LETTERS.map((ch, i) => {
            ...
            return (<PvElement as={"span"} key={i} style={{
          ...
          <PvElement as={"div"} style={{ position: 'relative', height: 6, width: 260, margin: '34px auto 0' }}>
            <PvElement as={"div"} style={{ position: 'absolute', inset: 0, borderRadius: 3, background: 'oklch(52% 0.115 65)', transform: `scaleX(${rule})` }}/>
            {ruleExt > 0 && ruleExtFade > 0 ? (<>
                <PvElement as={"div"} style={{ position: 'absolute', top: 2.5, height: 1, right: '100%', width: 190 * ruleExt, background: 'oklch(52% 0.115 65)', opacity: ruleExtFade }}/>
                <PvElement as={"div"} style={{ position: 'absolute', top: 2.5, height: 1, left: '100%', width: 190 * ruleExt, background: 'oklch(52% 0.115 65)', opacity: ruleExtFade }}/>
              </>) : null}
          </PvElement>
          <PvElement as={"div"} style={{ fontFamily: MONO, fontSize: 25, letterSpacing: '0.14em', color: 'oklch(50% 0.006 82)', marginTop: 30, opacity: tag, textTransform: 'uppercase' }}>
