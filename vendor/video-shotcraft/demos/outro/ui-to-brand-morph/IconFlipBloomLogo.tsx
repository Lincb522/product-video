...
const SmileLaptop: React.FC<{
    size: number;
}> = ({ size }) => (<PvElement as={"svg"} width={size} height={size} viewBox="0 0 40 40">
    <PvElement as={"rect"} x={7} y={6} width={26} height={20} rx={3.5} fill="#fff" stroke={G.ink} strokeWidth={3}/>
    <PvElement as={"circle"} cx={15.5} cy={13.5} r={2} fill={G.ink}/>
    <PvElement as={"circle"} cx={24.5} cy={13.5} r={2} fill={G.ink}/>
    <PvElement as={"path"} d="M14 18.5 Q20 23.5 26 18.5" stroke={G.ink} strokeWidth={2.8} fill="none" strokeLinecap="round"/>
    <PvElement as={"path"} d="M3.5 31.5 L36.5 31.5" stroke={G.ink} strokeWidth={3.6} strokeLinecap="round"/>
  </PvElement>);
// 5 瓣抽象花形 mark，bloom: 0(闭合竖线)→1(全开)
const FlowerMark: React.FC<{
    size: number;
    bloom: number;
}> = ({ size, bloom }) => {
    const pvArrayCopy = useMotionCopy();
    const petals = 5;
    return (<PvElement as={"svg"} width={size} height={size} viewBox="-50 -50 100 100">
      {Array.from(pvArrayCopy({ length: petals })).map((_, i) => {
            // 从竖直方向(-90°)向两侧展开到均匀分布
            ...
            return (<PvElement as={"ellipse"} key={i} cx={0} cy={-len / 2} rx={wid / 2} ry={len / 2} fill={INK} opacity={0.92} transform={`rotate(${angle + 90})`}/>);
        })}
      <PvElement as={"circle"} r={interpolate(bloom, [0, 1], [2, 9])} fill={G.ink}/>
    </PvElement>);
};
...
export const IconFlipBloomLogo: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: G.bg, alignItems: 'center', justifyContent: 'center' }}>
      <PvElement as={"div"} style={{ position: 'relative', width: 1920, height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {/* 图标 / mark 容器 */}
        <PvElement as={"div"} style={{
          ...
          {showIcon ? (<>
            ...
            </>) : (<PvElement as={"div"} style={{ transform: `scaleX(${markScaleX})` }}>
