...
export const InputMorphsIntoLogo: React.FC = () => {
    ...
    const dropSpring = (i: number) => spring({ frame: f - DROPS[i], fps, config: { damping: 12, stiffness: 110, mass: 0.85 } });
    // 落定呼吸：整瓣轻微 scale 脉动
    ...
    const WHITE = '#fdf6ee';
    return (<PvElement as={AbsoluteFill} style={{ background: BG, fontFamily: 'Helvetica, Arial, sans-serif', overflow: 'hidden' }}>
      <PvElement as={AbsoluteFill} style={{ transform: `scale(${breathe})`, transformOrigin: `${CX}px ${CY - 40}px` }}>
        {/* 主体：输入框 → 主胶囊 */}
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"div"} style={{
            ...
            Ready, set, go!
            {/* 光标闪烁 */}
            <PvElement as={"span"} style={{ opacity: Math.floor(f / 8) % 2 === 0 && f < FLY ? 1 : 0 }}>|</PvElement>
          </PvElement>
          {/* 发送键 */}
          <PvElement as={"div"} style={{
            marginLeft: 'auto',
            ...
            {/* 纸飞机三角 */}
            <PvElement as={"svg"} width={34} height={34} viewBox="0 0 34 34">
              <PvElement as={"path"} d="M3 17 L31 4 L20 30 L15 19 Z" fill={BG}/>
            </PvElement>
          </PvElement>
        </PvElement>

        {/* 大圆（第一粒，落在主胶囊左端形成泪滴组合） */}
        {f >= DROPS[0] && (<PvElement as={"div"} style={{
                position: 'absolute',
        ...
        {/* 竖胶囊（第二粒） */}
        {f >= DROPS[1] && (<PvElement as={"div"} style={{
                position: 'absolute',
        ...
        {/* 小圆（第三粒，压在竖胶囊顶端旁） */}
        {f >= DROPS[2] && (<PvElement as={"div"} style={{
                position: 'absolute',
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"svg"} width={40} height={44} viewBox="0 0 40 44">
          <PvElement as={"path"} d="M4 2 L4 34 L13 26 L19 40 L26 37 L20 23 L32 22 Z" fill="#ffffff" stroke={BG} strokeWidth={2}/>
        </PvElement>
      </PvElement>
    </PvElement>);
};
