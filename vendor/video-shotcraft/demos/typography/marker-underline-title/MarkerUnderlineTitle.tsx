...
export const MarkerUnderlineTitle: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: '#f4f4f2', alignItems: 'center', justifyContent: 'center' }}>
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"}>
          Meet the{' '}
          <PvElement as={"span"} style={{ fontStyle: 'italic', position: 'relative', display: 'inline-block' }}>
            new
            {/* 马克笔下划线：clip 从左到右揭示，保留笔形本身的粗细变化 */}
            <PvElement as={"svg"} width={LEN} height={44} viewBox={`0 0 ${LEN} 44`} style={{ position: 'absolute', left: -12, bottom: -20, overflow: 'visible' }}>
              <PvElement as={"defs"}>
                <PvElement as={"clipPath"} id={revealId}>
                  <PvElement as={"rect"} x={0} y={-20} width={drawE * (LEN + 6)} height={60}/>
                </PvElement>
              </PvElement>
              {draw > 0 && (<PvElement as={"path"} d={path} fill="#111111" clipPath={`url(#${revealId})`}/>)}
            </PvElement>
          </PvElement>
        </PvElement>
        <PvElement as={"div"}>Notion AI</PvElement>
      </PvElement>
    </PvElement>);
};
