...
export const WordRelayGeometry: React.FC = () => {
    ...
    return (<DesignStage bg="#07080c">
      ...
      {SLOTS.map(({ label, geom, t0, t1 }, i) => {
            ...
            return (<PvElement as={"div"} key={label} style={{ position: 'absolute', inset: 0, opacity: alive ? tin * (1 - tout) : 0 }}>
            <PvElement as={"svg"} viewBox="-240 -135 480 270" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
              {geom.map((g, k) => {
                    if (g.dash) {
                        // 虚线大圆：整体从 0.4 生长到 1 并持续慢转
                        const grow = seg(t, t0 + 0.01, t0 + 0.14, E.outCubic);
                        return (<PvElement as={"circle"} key={k} cx={g.x} cy={0} r={g.r} fill="none" stroke="#565e78" strokeWidth={1.1} strokeDasharray="5 7" opacity={grow * (1 - tout)} transform={`rotate(${-90 + t * 30}) scale(${lerp(grow, 0.4, 1)})`}/>);
                    }
                    // 实线圆：pathLength 归一化 trim 生长 / 出场反向消隐
                    const trim = seg(t, t0 + 0.02 + (g.d || 0), t0 + 0.14 + (g.d || 0), E.outCubic) -
                        seg(t, t1 - 0.06, t1, E.inQuad) * (isLast ? 0 : 1);
                    return (<PvElement as={"circle"} key={k} cx={g.x} cy={0} r={g.r} fill="none" stroke="#565e78" strokeWidth={1.1} pathLength={1} strokeDasharray="1" strokeDashoffset={1 - Math.max(0, trim)} transform="rotate(-90)"/>);
                })}
            </PvElement>
