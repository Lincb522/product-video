...
}> = ({ label, icon, style, }) => (<PvElement as={"div"} style={{
        display: 'inline-flex',
        alignItems: 'center',
        gap: 16,
...
export const PillSlotCycle: React.FC = () => {
    ...
    if (!isFinale && rel >= 0) {
        ...
        slot = (<PvElement as={"div"} style={{ position: 'relative', display: 'inline-block' }}>
        {/* 撑起槽宽度的隐形占位（当前 pill） */}
        <Pill label={incoming.label} icon={incoming.icon} style={{ visibility: 'hidden' }}/>
        {outgoing && outT < 1 && (<PvElement as={"div"} style={{
                    position: 'absolute',
                    left: 0,
                    top: 0,
        ...
        {lastOutT < 1 && (<PvElement as={"div"} style={{
                    position: 'absolute',
                    left: 0,
                    top: -8,
