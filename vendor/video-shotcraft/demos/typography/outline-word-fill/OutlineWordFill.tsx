...
export const OutlineWordFill: React.FC = () => {
    ...
    return (<DesignStage bg="#030303">
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: '#030303', overflow: 'hidden' }}>
        {/* 背景微尘 */}
        {DUST.map(({ size, opacity, left, top }, i) => (<PvElement as={"div"} key={i} style={{
                position: 'absolute',
                width: size,
                height: size,
                borderRadius: '50%',
        ...
        <PvElement as={"svg"} viewBox="-240 -135 480 270" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
          <PvElement as={"g"} transform={`scale(${lerp(shrink, 2.8, 1)}) rotate(${t * 18})`}>
            <PvElement as={"circle"} cx={0} cy={0} r={88} fill="none" stroke="#7d838e" strokeWidth={1} strokeDasharray="6 8" opacity={cin * 0.85}/>
          </PvElement>
          <PvElement as={"line"} y1={0} y2={0} x1={-240} x2={-240 + ext * 144} stroke="#7d838e" strokeWidth={1} strokeDasharray="5 7" opacity={ext}/>
          <PvElement as={"line"} y1={0} y2={0} x1={240} x2={240 - ext * 144} stroke="#7d838e" strokeWidth={1} strokeDasharray="5 7" opacity={ext}/>
        </PvElement>
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"div"} style={{
            ...
            color: '#fff',
            opacity: pop,
            textShadow: flash > 0.01 ? `0 0 ${flash * 16}px rgba(255,255,255,${flash * 0.45})` : 'none',
        }}>
            Faster
          </PvElement>
        </PvElement>
