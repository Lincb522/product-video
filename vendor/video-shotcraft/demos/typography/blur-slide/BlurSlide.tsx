...
}> = ({ words, tLine, gap, dy, style }) => (<PvElement as={"div"} style={{ display: 'flex', gap: '0.32em', ...style }}>
    {words.map((w, i) => {
...
export const BlurSlide: React.FC = () => {
    ...
    return (<DesignStage bg="#0a0b10">
      <PvElement as={"div"} style={{
        ...
        {/* 主标题先入场，副标题时间窗错后跟进 */}
        <Line words={H1_WORDS} tLine={seg(t, 0.06, 0.62)} gap={0.055} dy={40} style={{
            fontWeight: 800,
            fontSize: 34,
            lineHeight: 1.15,
        ...
        <Line words={H2_WORDS} tLine={seg(t, 0.34, 0.9)} gap={0.04} dy={26} style={{
            fontWeight: 400,
            fontSize: 14,
            lineHeight: 1.4,
            fontFamily: '-apple-system,sans-serif',
