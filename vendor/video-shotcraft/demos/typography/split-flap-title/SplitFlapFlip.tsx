...
const garble = (i: number, k: number) => CHARSET[Math.floor(rnd(i * 7.13 + k * 3.71 + 1) * CHARSET.length)];
...
const Half: React.FC<{
    ch: string;
    part: 'top' | 'bottom';
}> = ({ ch, part }) => (<PvElement as={"div"} style={{
        position: 'absolute',
        left: 0,
...
export const SplitFlapFlip: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: G.bg, overflow: 'hidden' }}>
      {/* 背景假页面压暗，突出翻牌板 */}
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, opacity: 0.3, filter: 'saturate(0.8)' }}>
      ...
      <PvElement as={AbsoluteFill} style={{ alignItems: 'center', justifyContent: 'center' }}>
        <PvElement as={"div"} style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          {pvCopy(TEXT).split('').map((ch, idx) => {
            if (ch === ' ') {
                return <PvElement as={"div"} key={idx} style={{ width: 52 }}/>;
            }
            const i = letterIdx++;
