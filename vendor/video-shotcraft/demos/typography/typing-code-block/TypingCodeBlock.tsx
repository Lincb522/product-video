...
const Panel: React.FC<{
...
}> = ({ x, label, children, }) => (<PvElement as={"div"} style={{
        position: 'absolute',
        left: `${x}%`,
        top: '14%',
        width: '45%',
    ...
    <PvElement as={"div"} style={{ fontSize: 8, letterSpacing: 2, color: '#4a5270', marginBottom: 6 }}>{label}</PvElement>
    {children}
  </PvElement>);
    ...
    return (<DesignStage bg="#0a0b10">
      ...
      <Panel x={3.5} label="LINE FADE-IN">
        {LINES.map((line, i) => {
            ...
            return (<PvElement as={"div"} key={i} style={{ opacity: k, transform: `translateY(${(1 - k) * 8}px)` }}>
              {line.map(([txt, color], j) => (<PvElement as={"span"} key={j} style={{ color }}>
                  {txt}
                </PvElement>))}
            </PvElement>);
        })}
      ...
      <Panel x={51.5} label="CHAR TYPING">
        {LINES.map((_, row) => (<PvElement as={"div"} key={row} style={{ minHeight: '1.9em' }}>
            {FLAT.map((c, i) => c.row === row ? (<PvElement as={"span"} key={i} style={{
                    color: c.color,
                    // 光标位字符以底色块形式提示（保持可见）
