...
const Skeleton: React.FC<{
    ...
    return (<PvElement as={"div"} style={{ display: 'flex', flexDirection: 'column', gap: 34 }}>
      {blocks.map((b, i) => {
...
const DemoteScene: React.FC<{
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: G.bg }}>
      {/* 内容骨架区 */}
      <PvElement as={"div"} style={{ position: 'absolute', left: 150, top: 210 }}>
        <Skeleton t={growT}/>
      </PvElement>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{
          ...
          {withSelection && selWidth > 0 && (<PvElement as={"div"} style={{
                position: 'absolute',
                left: `${selLeft}%`,
                top: 8,
                width: `${selWidth}%`,
          ...
          <PvElement as={"span"} style={{ position: 'relative' }}>{title}</PvElement>
        </PvElement>
      </PvElement>
    </PvElement>);
};
export const TitleDemoteToLabel: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill}>
      ...
      <PvElement as={AbsoluteFill} style={{ background: '#fff', opacity: flash, pointerEvents: 'none' }}/>
    </PvElement>);
};
