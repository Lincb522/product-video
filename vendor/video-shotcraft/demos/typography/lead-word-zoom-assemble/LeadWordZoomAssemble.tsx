...
import { AbsoluteFill, continueRender, delayRender, Easing, getRemotionEnvironment, interpolate, useCurrentFrame } from 'remotion';
...
const TextReveal: React.FC = () => {
    ...
    const words = pvCopy(TEXT).split(' ').filter(Boolean);
    ...
    return (<PvElement as={AbsoluteFill} style={{ alignItems: 'center', justifyContent: 'center' }}>
      <PvElement as={"span"} ref={lineRef} style={{
            position: 'relative',
            ...
            scale: interpolate(frame, [0, HOLD_DURATION], [INITIAL_SCALE, INITIAL_SCALE * PUSH_SCALE], {
                ...
                interpolate(frame, [zoomStart, zoomStart + RECEDE_DURATION], [0, 1 - INITIAL_SCALE * PUSH_SCALE], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: ZOOM_EASE }),
            translate: `${interpolate(frame, [zoomStart, zoomStart + ASSEMBLE_DURATION], [slideDistance, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: ZOOM_EASE })}px`,
            opacity: ready ? 1 : 0,
        ...
        {words.map((word, i) => {
            ...
            return (<Fragment key={i}>
              <PvElement as={"span"} ref={isLead ? leadRef : undefined} style={{
                    display: 'inline-block',
                    opacity,
                    color: word === HIGHLIGHT_WORD ? ACCENT : undefined,
                    translate: isLead
                        ? undefined
                        : `${interpolate(frame, [pushStart, pushStart + WORD_DURATION], [WORD_PUSH * FONT_SIZE, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: WORD_EASE })}px`,
                }}>
                {word}
              </PvElement>
        ...
        <PvElement as={"span"} ref={baselineRef} style={{ display: 'inline-block', width: 0, height: 0 }}/>
      </PvElement>
    </PvElement>);
};
export const LeadWordZoomAssemble: React.FC = () => {
    ...
    const crash = interpolate(frame, [durationInFrames - CRASH_FRAMES, durationInFrames - 1], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.in(Easing.quad) });
    return (<PvElement as={AbsoluteFill} style={{ background: MESH_BG, fontFamily: SANS }}>
      <PvElement as={AbsoluteFill} style={{
        ...
        <PvElement as={AbsoluteFill} style={{ transform: `translateY(${lift * LIFT_DISTANCE}px)` }}>
          <TextReveal />
        </PvElement>
