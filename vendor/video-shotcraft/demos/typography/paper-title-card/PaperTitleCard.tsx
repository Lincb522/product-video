...
const DigitColumn: React.FC<{
    ...
    lineH: number;
    color: string;
}> = ({ ch, delay, lineH, color }) => {
    const pvCopy = useMotionCopy();
    const frame = useCurrentFrame();
    if (ch < '0' || ch > '9') {
        return <PvElement as={"span"} style={{ fontSize: 26, lineHeight: `${lineH}px`, color }}>{ch}</PvElement>;
    }
    ...
    return (<PvElement as={"span"} style={{ display: 'inline-block', height: lineH }}>
      <PvElement as={"span"} style={{ display: 'block', transform: `translateY(${-offset}px)` }}>
        {pvCopy((DIGITS + DIGITS)).split('').map((d, j) => (<PvElement as={"span"} key={j} style={{
                display: 'block',
                fontSize: 26,
                lineHeight: `${lineH}px`,
                color,
...
const DigitRoll: React.FC<{
    ...
    return (<PvElement as={"span"} style={{ display: 'inline-flex', overflow: 'hidden', height: lineH, verticalAlign: 'bottom' }}>
      {pvCopy(value).split('').map((c, i) => (<DigitColumn key={i} ch={c} delay={delay + i * 4} lineH={lineH} color={color}/>))}
    </PvElement>);
};
export const PaperTitleCard: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{
      ...
      <PvElement as={"div"} style={{ textAlign: 'center', maxWidth: 1500 }}>
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"span"}>{SUB}</PvElement>
        </PvElement>
      </PvElement>
    </PvElement>);
};
