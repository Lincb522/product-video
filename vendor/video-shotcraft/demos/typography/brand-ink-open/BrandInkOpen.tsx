...
export const BrandInkOpen: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: '#faf7f2', justifyContent: 'center', alignItems: 'center' }}>
      <PvElement as={"div"} style={{ textAlign: 'center', opacity: brandOpacity, transform: `translateY(${groupY}px) scale(${groupScale})` }}>
        {/* crosshair drawn by an invisible amber pen */}
        <PvElement as={"svg"} width={64} height={64} viewBox="0 0 64 64" style={{ display: 'block', margin: '0 auto 34px', opacity: crossFade }}>
          <PvElement as={"line"} x1={32} y1={2} x2={32} y2={62} stroke={AMBER} strokeWidth={5} strokeLinecap="round" pathLength={100} strokeDasharray={100} strokeDashoffset={vDraw}/>
          <PvElement as={"line"} x1={2} y1={32} x2={62} y2={32} stroke={AMBER} strokeWidth={5} strokeLinecap="round" pathLength={100} strokeDasharray={100} strokeDashoffset={hDraw}/>
        </PvElement>
        ...
        <PvElement as={"div"} style={{
          ...
          {pvCopy(WORDMARK).split('').map((ch, i) => {
            const delay = 10 + i * 3;
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"span"} style={{ whiteSpace: 'pre' }}>{KICKER.slice(0, kickChars)}</PvElement>
          <PvElement as={"span"} style={{
