...
export const PillChipSlotCycleHandled: React.FC = () => {
    ...
    useLayoutEffect(() => {
        ...
        if (!spans)
            return;
        setWidths(WORDS.map((o, i) => ((spans[i] as HTMLElement).offsetWidth || o.w.length * 13) + 74));
    }, []);
    // 三次切换：0.25 / 0.47 / 0.69，每次用 0.12 的 inOutCubic 推进一格
    let pos = 0;
    for (const s0 of [0.25, 0.47, 0.69])
        pos += seg(t, s0, s0 + 0.12, E.inOutCubic);
    ...
    return (<DesignStage bg="#fbfbfd">
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} ref={measRef} style={{ position: 'absolute', visibility: 'hidden' }}>
          {WORDS.map((o) => (<PvElement as={"span"} key={o.w} style={{ whiteSpace: 'nowrap', font: FONT }}>
              {o.w}
            </PvElement>))}
        </PvElement>
        <PvElement as={"div"} style={SIDE}>Your</PvElement>
        <PvElement as={"div"} style={{ position: 'relative', flex: 'none' }}>
          ...
          <PvElement as={"div"} style={{
            ...
            transform: `translateY(${-pos * 48}px)`,
              ...
              {WORDS.map(({ w, e }) => (<PvElement as={"div"} key={w} style={{
                height: 48,
                display: 'flex',
                alignItems: 'center',
                gap: 9,
                ...
                whiteSpace: 'nowrap',
                  ...
                  <PvElement as={"span"} style={{ fontSize: 18 }}>{e}</PvElement>
                  <PvElement as={"span"} style={{ font: FONT, color: '#fff' }}>{w}</PvElement>
                </PvElement>))}
            </PvElement>
          </PvElement>
          {/* 胶囊外上下的灰色幽灵项 */}
          <PvElement as={"div"} style={ghost(-38)}>{near > 0 ? WORDS[near - 1].w : ''}</PvElement>
          <PvElement as={"div"} style={ghost(58)}>{near < WORDS.length - 1 ? WORDS[near + 1].w : ''}</PvElement>
        </PvElement>
        <PvElement as={"div"} style={SIDE}>Handled</PvElement>
      </PvElement>
    </DesignStage>);
};
