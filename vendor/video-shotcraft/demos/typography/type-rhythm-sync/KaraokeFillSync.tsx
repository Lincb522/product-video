...
const KaraokeWord: React.FC<{
    ...
    return (<PvElement as={"span"} style={{ position: 'relative', display: 'inline-block' }}>
      {/* 底层：浅灰未读字 */}
      <PvElement as={"span"} style={{ color: G.line }}>{word.text}</PvElement>
      {/* 上层：深字按进度从左到右揭开 */}
      ...
      {active && (<PvElement as={"span"} style={{
                position: 'absolute',
                left: 0,
                bottom: -14,
                width: `${p * 100}%`,
...
export const KaraokeFillSync: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{
      ...
      {LINES.map((words, li) => (<PvElement as={"div"} key={li} style={{ display: 'flex', gap: 48 }}>
