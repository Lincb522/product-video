...
export const TextOnPath: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      ...
      <PvElement as={"svg"} width={1920} height={1080} style={{ position: 'absolute', inset: 0 }}>
        <PvElement as={"path"} d={`M ${P0.x} ${P0.y} C ${P1.x} ${P1.y}, ${P2.x} ${P2.y}, ${P3.x} ${P3.y}`} fill="none" stroke={G.bar} strokeWidth={3} pathLength={1} strokeDasharray={1} strokeDashoffset={1 - evolve}/>
      </PvElement>
      {pvCopy(TEXT).split('').map((ch, i) => {
            if (ch === ' ')
                return null;
