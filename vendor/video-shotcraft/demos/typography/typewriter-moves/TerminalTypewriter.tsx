...
const TerminalWindow: React.FC<{
    chars: number;
    cursorOn: boolean;
}> = ({ chars, cursorOn }) => (<PvElement as={"div"} style={{
    ...
    <PvElement as={"div"} style={{
      ...
      {['#6a6a68', '#8f8f8d', '#b5b5b3'].map((c, i) => (<PvElement as={"div"} key={i} style={{ width: 16, height: 16, borderRadius: 8, background: c }}/>))}
      <PvElement as={"div"} style={{ margin: '0 auto', height: 10, width: 200, background: '#4a4a48', borderRadius: 5 }}/>
      <PvElement as={"div"} style={{ width: 72 }}/>
    </PvElement>
    {/* 内容区 */}
    <PvElement as={"div"} style={{
      ...
      {/* 一行历史输出做上下文 */}
      <PvElement as={"div"} style={{ color: '#7a7a78', fontSize: 32, marginBottom: 18 }}>~/acme-app (main)</PvElement>
      <PvElement as={"div"} style={{ display: 'flex', alignItems: 'center', whiteSpace: 'pre' }}>
        <PvElement as={"span"} style={{ color: '#9f9f9d' }}>{'$ '}</PvElement>
        <PvElement as={"span"}>{CMD.substring(0, chars)}</PvElement>
