...
export const TypewriterErrorRetype: React.FC = () => {
    ...
    const shown = TEXT1.slice(0, Math.max(KEEP, n1 - removed)).slice(0, n1) +
        TEXT2.slice(0, n2);
    const chars = pvCopy(shown).split('');
    return (<PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{
        ...
        {chars.map((c, i) => (<PvElement as={"span"} key={i} style={{
        ...
        {cursorOn(f) && (<PvElement as={"span"} style={{
                display: 'inline-block',
                width: 7,
                height: 100,
                marginLeft: 4,
