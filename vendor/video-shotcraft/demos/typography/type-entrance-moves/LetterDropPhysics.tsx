...
const dropY = (t: number): number => {
    if (t <= 0)
        return -DROP;
    if (t < T_FALL)
        return -DROP + DROP * (t / T_FALL) ** 2; // 重力加速
    if (t < T_FALL + T_B1) {
...
export const LetterDropPhysics: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, overflow: 'hidden', position: 'relative' }}>
      {/* 地板线：字符落点的视觉基线 */}
      <PvElement as={"div"} style={{ position: 'absolute', left: LEFT - 60, top: FLOOR_Y, width: WORD_W + 120, height: 6, background: G.bar, borderRadius: 3 }}/>
