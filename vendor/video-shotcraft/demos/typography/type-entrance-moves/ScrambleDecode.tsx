...
export const ScrambleDecode: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{
        ...
        {chars.map((ch, i) => {
            if (ch === ' ') {
                return <PvElement as={"span"} key={i} style={{ display: 'inline-block', width: '0.7em' }}/>;
            }
      ...
      <PvElement as={"div"} style={{ width: 900, height: 10, background: G.line, borderRadius: 5, overflow: 'hidden' }}>
        <PvElement as={"div"} style={{
