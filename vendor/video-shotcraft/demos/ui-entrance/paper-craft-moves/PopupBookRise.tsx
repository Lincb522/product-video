...
export const PopupBookRise: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: '#dddddb', position: 'relative', overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, perspective: 2600, perspectiveOrigin: '50% 30%' }}>
        <PvElement as={"div"} style={{
            position: 'absolute',
            inset: 0,
          ...
          <PvElement as={"div"} style={{ position: 'absolute', inset: 0, background: G.bg, boxShadow: '0 40px 80px rgba(0,0,0,0.25)' }}>
            <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 220, background: G.side, padding: '28px 22px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <PvElement as={"div"} style={{ position: 'absolute', left: 220, right: 0, top: 0, height: 72, background: G.panel, borderBottom: `2px solid ${G.line}`, display: 'flex', alignItems: 'center', padding: '0 32px', gap: 20, boxSizing: 'border-box' }}>
          ...
          {/* 6 张纸片卡沿底边立起 */}
          {Array.from(pvArrayCopy({ length: 6 })).map((_, i) => (<PageCard key={i} i={i} frame={frame}/>))}
        </PvElement>
      </PvElement>
    </PvElement>);
};
