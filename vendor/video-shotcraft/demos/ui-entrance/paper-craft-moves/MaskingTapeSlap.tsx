...
const frozen = (f: number, raw: (x: number) => number): number => f <= SLAP2
    ? raw(f)
...
const Tape: React.FC<{
    ...
    if (frame < land - APPROACH)
        return null; // 条件卸载：拍上前真不存在
...
export const MaskingTapeSlap: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      ...
      <PvElement as={"div"} style={{
        ...
        <Card w={CARD_W} h={CARD_H} seed={3} style={{ boxShadow: `0 ${shOff}px ${shBlur}px rgba(0,0,0,${shAlpha})` }}/>
      </PvElement>
