...
export const RowEmbed: React.FC = () => {
    ...
    return (<PageCam2D src="textures/live/detail-full.png" pageH={DETAIL_H} keys={DETAIL_CAM}>
      {rows.map((r, i) => {
            const cue = 12 + i * 9;
            const land = cue + 12;
            ...
            if (frame >= cue && frame < cue + 16) {
                ...
                const air = 1 - p;
                flyer = (<PvElement as={"div"} key={`row-${i}`} style={{
            ...
            if (frame >= land && frame < land + 8) {
                ...
                seam = (<PvElement as={"div"} key={`seam-${i}`} style={{
                        position: 'absolute',
            ...
            {patchOpacity > 0 ? (<PvElement as={"div"} key={`patch-${i}`} style={{
