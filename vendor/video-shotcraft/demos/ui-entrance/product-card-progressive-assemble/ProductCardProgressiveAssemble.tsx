...
export const ProductCardProgressiveAssemble: React.FC = () => {
    ...
    return (<DesignStage bg="#0d0e13" raster="zoom">
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{
          ...
          <PvElement as={"div"} style={{
            ...
            {/* 拉链 */}
            <PvElement as={"div"} style={{ position: 'absolute', left: '49%', top: '20%', width: '2%', height: '58%', background: '#525a70' }}/>
          </PvElement>
        </PvElement>
        {/* 右列 */}
        <PvElement as={"div"} style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          {/* 标题（f=4） */}
          <PvElement as={"div"} style={{ fontSize: 19, fontWeight: 800, color: '#17181c', letterSpacing: '-.3px', ...fieldStyle(t, 4) }}>
            Sample Product Title
          </PvElement>
          {/* breadcrumb pills（f=8 起每 2 帧一个 pop） */}
          <PvElement as={"div"} style={{ display: 'flex', gap: 6, margin: '8px 0 10px' }}>
            {['Category', 'Subgroup', 'Detail'].map((txt, i) => (<PvElement as={"div"} key={txt} style={{
                fontSize: 9,
                fontWeight: 600,
                color: '#5a5e6b',
                background: '#e9e7e1',
          ...
          <PvElement as={"div"} style={{ display: 'flex', alignItems: 'baseline', gap: 9, marginBottom: 11, ...fieldStyle(t, 16) }}>
          ...
          <PvElement as={"div"} style={{ fontSize: 10.5, lineHeight: 1.75, color: '#494d58' }}>
            {LINES.map((segs, li) => (<PvElement as={"div"} key={li} style={{ whiteSpace: 'nowrap', ...fieldStyle(t, 30 + li * 2) }}>
                {segs.map(([txt, isMark], si) => !isMark ? (<PvElement as={"span"} key={si}>{txt}</PvElement>) : (<PvElement as={"span"} key={si} style={{ position: 'relative', display: 'inline-block' }}>
                      <PvElement as={"span"} style={{
                    position: 'absolute',
                    left: -2,
                    right: -2,
                    ...
                    transformOrigin: 'left center',
                      ...
                      <PvElement as={"span"} style={{ position: 'relative', fontWeight: 700, color: '#26282f' }}>{txt}</PvElement>
                    </PvElement>))}
              </PvElement>))}
          </PvElement>
          {/* 色卡（f=40 起每 2 帧一个 pop） */}
          <PvElement as={"div"} style={{ display: 'flex', gap: 7, marginTop: 'auto' }}>
            {['#191b20', '#8b8f99', '#3a5b8c'].map((cclr, i) => (<PvElement as={"div"} key={cclr} style={{
                width: 16,
                height: 16,
                borderRadius: 4,
                background: cclr,
