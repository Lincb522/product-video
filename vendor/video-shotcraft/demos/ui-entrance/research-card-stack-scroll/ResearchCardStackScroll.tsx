      ...
      <PvElement as={"div"} style={{
        ...
        {TITLES.map((tt, i) => {
            ...
            return (<PvElement as={"div"} key={i} style={{
              ...
              <PvElement as={"div"} style={{ position: 'absolute', left: 14, top: 40, width: 266, opacity: i === focus ? 1 : 0 }}>
                <PvElement as={"div"} style={{ font: `500 8px/1 ${FONT}`, color: ORANGE_SOFT, letterSpacing: '.3px' }}>
                  {`A. Author, B. Writer, C. Reader · preprint:24${10 + i}.0${i % 9}${i % 7}`}
                </PvElement>
                {[0, 1, 2, 3].map((k) => (<PvElement as={"div"} key={k} style={{
                        marginTop: k ? 5 : 9,
