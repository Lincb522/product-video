...
const ease = (frame: number, start: number, end: number, from = 0, to = 1) => interpolate(frame, [start, Math.max(start + 1, end)], [from, to], {
    ...CLAMP,
    easing: Easing.bezier(0.16, 1, 0.3, 1),
});
const hingeEase = (frame: number, start: number, end: number) => interpolate(frame, [start, Math.max(start + 1, end)], [0, 1], {
    ...CLAMP,
...
export const PlatformHingeRise: React.FC = () => {
    ...
    return (<DesignStage bg="#f4f4f1" raster="zoom">
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ position: 'absolute', inset: 0, clipPath: 'inset(0 0 79px 0)' }}>
