...
export const AvatarBracketCarousel: React.FC = () => {
    ...
    return (<DesignStage bg="#0b0c12">
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ color: '#eef1f8', fontWeight: 800, fontSize: 34, letterSpacing: -0.5 }}>Your</PvElement>

        {/* 中央对焦框 + 头像列 */}
        <PvElement as={"div"} style={{ position: 'relative', width: 92, height: 92, flex: 'none' }}>
          <PvElement as={"div"} style={{ position: 'absolute', left: '50%', top: '50%', width: 0, height: 0 }}>
            {AV.map((a, k) => {
            const d = Math.abs(k - pos);
            const sc = Math.max(0.5, 1 - d * 0.38);
            const op = Math.max(0, 1 - d * 0.62);
            return (<PvElement as={"div"} key={k} style={{
                    position: 'absolute',
                    left: -29,
                    top: -29,
                    width: 58,
          ...
          <PvElement as={"div"} style={{
            ...
            <PvElement as={"svg"} width={92} height={92} viewBox="0 0 92 92">
              {CORNERS.map((d, i) => (<PvElement as={"path"} key={i} d={d} fill="none" stroke={ACCENT} strokeWidth={4} strokeLinecap="round"/>))}
            </PvElement>
          </PvElement>

          {/* 角色标签（只随对焦位置换透明度，位置不动） */}
          {AV.map((a, k) => (<PvElement as={"div"} key={k} style={{
                position: 'absolute',
                left: '50%',
                top: '50%',
        ...
        <PvElement as={"div"} style={{ color: '#eef1f8', fontWeight: 800, fontSize: 34, letterSpacing: -0.5 }}>teammates</PvElement>
      </PvElement>
    </DesignStage>);
};
