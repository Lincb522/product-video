 ...
 */
export const VerticalTicker: React.FC<VerticalTickerProps> = ({ columns, backgroundColor = '#000', maskHeight = 200, tiltDeg = 20, perspective = 1000, scale = 1.2, columnWidth = 400, gap = 30, }) => {
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor, overflow: 'hidden' }}>
      <PvElement as={"div"} style={{
        ...
        {columns.map((col, idx) => (<Column key={idx} {...col} width={columnWidth} gap={gap}/>))}
      </PvElement>
...
const Column: React.FC<TickerColumn & {
    width: number;
    gap: number;
}> = ({ items, durationInSeconds, direction, width, gap, }) => {
    ...
    return (<PvElement as={"div"} style={{ width, height: '100%', display: 'flex', flexDirection: 'column' }}>
      <PvElement as={"div"} style={{
            display: 'flex',
        ...
        {[...items, ...items].map((node, i) => (<PvElement as={"div"} key={i} style={{ marginBottom: gap }}>
            {node}
          </PvElement>))}
      </PvElement>
    </PvElement>);
