...
const Cell: React.FC<{
    ...
    return (<PvElement as={"div"} style={{ position: 'absolute', left: x, top: y, width: CELL_W, height: CELL_H }}>
      ...
      {/* 边框流光：SVG rect 描边一圈 */}
      {draw > 0 && (<PvElement as={"svg"} width={CELL_W} height={CELL_H} viewBox={`0 0 ${CELL_W} ${CELL_H}`} style={{ position: 'absolute', left: 0, top: ty, overflow: 'visible' }}>
          <PvElement as={"rect"} x={2} y={2} width={CELL_W - 4} height={CELL_H - 4} rx={14} fill="none" stroke={AMBER} strokeWidth={4} pathLength={100} strokeDasharray={100} strokeDashoffset={100 * (1 - draw)} opacity={strokeFade} style={{ filter: `drop-shadow(0 0 ${6 + jitter * 4}px ${AMBER})` }}/>
        </PvElement>)}
    </PvElement>);
};
export const BentoLightUp: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: BG, overflow: 'hidden' }}>
      <PvElement as={"div"} style={{
            position: 'absolute',
            inset: 0,
        ...
        <PvElement as={"div"} style={{ position: 'absolute', left: LEFT, top: TOP - 110, opacity: titleLit, filter: 'invert(1)' }}>
          <TitleBlock text="Features" size={64}/>
        </PvElement>
        {Array.from(pvArrayCopy({ length: 6 })).map((_, i) => (<Cell key={i} i={i} frame={frame}/>))}
      </PvElement>
    </PvElement>);
};
