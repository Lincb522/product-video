...
const HubPanel: React.FC<{
    ...
    <PvElement as={"div"} style={{ flex: 2 }}>
      <PvElement as={"div"} style={{ fontSize: 27, fontWeight: 600, color: '#2f2f38' }}>Enterprise MQLs</PvElement>
      <PvElement as={"div"} style={{ display: 'flex', gap: 16, marginTop: 12 }}>
        {['All', 'Tasks', 'Docs', 'Whiteboards', 'Dashboards', 'Files', 'Chat', 'People'].map((t, i) => (<PvElement as={"div"} key={t} style={{ fontSize: 12, color: i === 0 ? '#5b55c8' : '#98989f', fontWeight: i === 0 ? 700 : 400 }}>
            {t}
          </PvElement>))}
      </PvElement>
      <PvElement as={"div"} style={{ fontSize: 12, color: '#8b8b92', marginTop: 14 }}>Recent</PvElement>
      {LIST.map((it, i) => (<PvElement as={"div"} key={i} style={{ display: 'flex', gap: 11, alignItems: 'center', marginTop: 13.5 }}>
          <PvElement as={"div"} style={{ width: 22, height: 22, borderRadius: 6, background: it.icon, opacity: 0.85 }}/>
          <PvElement as={"div"}>
            <PvElement as={"div"} style={{ fontSize: 13.5, fontWeight: 600, color: '#3c3c44' }}>{it.title}</PvElement>
            <PvElement as={"div"} style={{ fontSize: 11, color: '#a2a2a8', marginTop: 1 }}>{it.sub}</PvElement>
          </PvElement>
        </PvElement>))}
    </PvElement>
    <PvElement as={"div"} style={{ flex: 1, borderLeft: '1px solid #e7e7e5', paddingLeft: 22 }}>
      ...
      <PvElement as={"div"} style={{ fontSize: 10.5, color: '#adadb2', marginTop: 24, letterSpacing: 1 }}>QUICK FILTERS</PvElement>
      {['Assigned to Me', 'Created by Me'].map((t) => (<PvElement as={"div"} key={t} style={{ fontSize: 13.5, color: '#55555c', marginTop: 11 }}>{t}</PvElement>))}
      <PvElement as={"div"} style={{ fontSize: 10.5, color: '#adadb2', marginTop: 24, letterSpacing: 1 }}>TASK FILTERS</PvElement>
      {['Open', 'Closed', 'Archived'].map((t) => (<PvElement as={"div"} key={t} style={{ fontSize: 13.5, color: '#55555c', marginTop: 11 }}>{t}</PvElement>))}
    </PvElement>
  </PvElement>);
...
const FrontPanel: React.FC<{
    ...
    <PvElement as={"div"} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <PvElement as={"div"} style={{ width: 26, height: 26, borderRadius: 7, background: '#4a9fd8', opacity: 0.9 }}/>
      <PvElement as={"div"} style={{ fontSize: 26, fontWeight: 600, color: '#2f2f38' }}>Q3 Enterprise Deal</PvElement>
    </PvElement>
    <PvElement as={"div"} style={{ display: 'flex', gap: 14, marginTop: 10 }}>
      {['Revenue', 'Pipeline', 'Q3 Quota'].map((t) => (<PvElement as={"div"} key={t} style={{
            fontSize: 11.5,
            color: '#6a6a70',
            background: '#efeff0',
            borderRadius: 6,
    ...
    <PvElement as={"div"} style={{ height: 1, background: '#e8e8e6', marginTop: 18 }}/>
    {/* 文档灰条段落 */}
    {[420, 700, 660, 540, 0, 690, 630, 380, 0, 580, 640, 460].map((w, i) => w === 0 ? (<PvElement as={"div"} key={i} style={{ height: 14 }}/>) : (<PvElement as={"div"} key={i} style={{
            width: w,
            height: 13,
            borderRadius: 6,
...
const Tile: React.FC<{
    ...
    const glyph = (() => {
        switch (kind) {
            case 'figma':
                return (<PvElement as={"svg"} width={46} height={46} viewBox="0 0 46 46">
            <PvElement as={"circle"} cx={16} cy={9} r={7} fill="#f24e1e"/>
            <PvElement as={"circle"} cx={30} cy={9} r={7} fill="#ff7262"/>
            <PvElement as={"circle"} cx={16} cy={23} r={7} fill="#a259ff"/>
            <PvElement as={"circle"} cx={30} cy={23} r={7} fill="#1abcfe"/>
            <PvElement as={"circle"} cx={16} cy={37} r={7} fill="#0acf83"/>
          </PvElement>);
            case 'github':
                return <PvElement as={"div"} style={{ width: 46, height: 46, borderRadius: '50%', background: '#24292f' }}/>;
            case 'salesforce':
                return (<PvElement as={"svg"} width={56} height={40} viewBox="0 0 56 40">
            <PvElement as={"ellipse"} cx={22} cy={22} rx={14} ry={11} fill="#00a1e0"/>
            <PvElement as={"ellipse"} cx={36} cy={18} rx={13} ry={10} fill="#00a1e0"/>
            <PvElement as={"ellipse"} cx={30} cy={26} rx={16} ry={10} fill="#00a1e0"/>
          </PvElement>);
            case 'gdrive':
                return (<PvElement as={"svg"} width={48} height={42} viewBox="0 0 48 42">
            <PvElement as={"path"} d="M16 2 L32 2 L48 30 L40 42 L8 42 L0 30 Z" fill="none"/>
            <PvElement as={"path"} d="M16 2 L32 2 L20 24 L4 24 Z" fill="#34a853" transform="translate(2,2)"/>
            <PvElement as={"path"} d="M32 2 L46 28 L30 28 L18 6 Z" fill="#fbbc04" transform="translate(0,2)"/>
            <PvElement as={"path"} d="M6 28 L42 28 L36 38 L12 38 Z" fill="#4285f4"/>
          </PvElement>);
            default: // dropbox
                return (<PvElement as={"svg"} width={48} height={42} viewBox="0 0 48 42">
            <PvElement as={"path"} d="M12 0 L24 8 L12 16 L0 8 Z" fill="#0061ff"/>
            <PvElement as={"path"} d="M36 0 L48 8 L36 16 L24 8 Z" fill="#0061ff"/>
            <PvElement as={"path"} d="M12 18 L24 26 L12 34 L0 26 Z" fill="#0061ff"/>
            <PvElement as={"path"} d="M36 18 L48 26 L36 34 L24 26 Z" fill="#0061ff"/>
          </PvElement>);
        }
    })();
...
export const IntegrationHubMap: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ background: '#08070c' }}>
      {/* 暗紫底噪 */}
      ...
      {RECTS.map((r, i) => {
            ...
            return (<PvElement as={"div"} key={i} style={{
                    position: 'absolute',
                    left: r.x,
                    top: r.y,
                    width: r.w,
      ...
      {/* 光管层 */}
      <PvElement as={"svg"} width={1920} height={1080} style={{ position: 'absolute', inset: 0, opacity: mapIn }}>
        <PvElement as={"defs"}>
          ...
          {PIPES.map((p, i) => {
            ...
            return (<PvElement as={"linearGradient"} key={i} id={`rainbow-${uid}-${i}`} gradientUnits="userSpaceOnUse" x1={x1} y1={y1} x2={x2} y2={y2}>
                <PvElement as={"stop"} offset="0%" stopColor="#ffe14d"/>
                <PvElement as={"stop"} offset="28%" stopColor="#ff8a5a"/>
                <PvElement as={"stop"} offset="52%" stopColor="#ff5ad0"/>
                <PvElement as={"stop"} offset="76%" stopColor="#b46bff"/>
                <PvElement as={"stop"} offset="100%" stopColor="#5ad0ff"/>
              </PvElement>);
        })}
          {/* userSpaceOnUse：纯水平/垂直直线管的 bbox 为零，百分比滤镜区域会
            坍缩成 0 导致整条管不渲染（github/salesforce/dropbox 三管消失） */}
          <PvElement as={"filter"} id={`pipeGlow-${uid}`} filterUnits="userSpaceOnUse" x="0" y="0" width="1920" height="1080">
            <PvElement as={"feGaussianBlur"} stdDeviation="8" result="b"/>
            <PvElement as={"feMerge"}>
              <PvElement as={"feMergeNode"} in="b"/>
              <PvElement as={"feMergeNode"} in="SourceGraphic"/>
            </PvElement>
          </PvElement>
        </PvElement>
        {PIPES.map((p, i) => {
            ...
            return (<PvElement as={"g"} key={i} filter={`url(#pipeGlow-${uid})`}>
              <PvElement as={"path"} d={p.path} fill="none" stroke={`url(#rainbow-${uid}-${i})`} strokeWidth={17} strokeLinecap="round" strokeDasharray={`${dashOn} ${p.len + 60}`} opacity={0.92 * pulse}/>
              <PvElement as={"path"} d={p.path} fill="none" stroke="rgba(255,255,255,0.9)" strokeWidth={5} strokeLinecap="round" strokeDasharray={`${dashOn} ${p.len + 60}`} opacity={0.85 * pulse}/>
              {grow >= 1 && flowIn > 0 && (<>
                  {/* 输送流：连续多段亮脉冲沿管流动（间距 46，段长 20） */}
                  <PvElement as={"path"} d={p.path} fill="none" stroke="#ffffff" strokeWidth={11} strokeLinecap="round" strokeDasharray="18 56" strokeDashoffset={flowOffset} opacity={0.95 * flowIn}/>
                  {/* 输送流柔光晕 */}
                  <PvElement as={"path"} d={p.path} fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth={24} strokeLinecap="round" strokeDasharray="18 56" strokeDashoffset={flowOffset} opacity={0.55 * flowIn}/>
                </>)}
            </PvElement>);
        })}
      </PvElement>
      ...
      <PvElement as={AbsoluteFill} style={{ justifyContent: 'center', alignItems: 'center', perspective: 1500 }}>
        <PvElement as={"div"} style={{
      ...
      {bloom > 0.02 && (<PvElement as={AbsoluteFill} style={{ pointerEvents: 'none' }}>
          <PvElement as={"div"} style={{
                position: 'absolute',
                left: 300,
                top: 60,
