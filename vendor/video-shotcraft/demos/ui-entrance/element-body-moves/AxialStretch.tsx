...
const posAt = (f: number, start: number, targetX: number): number => interpolate(f, [start, start + FLIGHT], [START_X, targetX], {
    easing: flightEase,
...
export const AxialStretch: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, position: 'relative', overflow: 'hidden' }}>
      <PvElement as={"div"} style={{ position: 'absolute', top: 120, width: '100%', textAlign: 'center', opacity: titleOp }}>
        <TitleBlock text="AXIAL STRETCH" size={72}/>
      </PvElement>
      {/* 落位虚线槽，标出目标位置 */}
      {[0, 1, 2].map((i) => (<PvElement as={"div"} key={`slot-${i}`} style={{
                position: 'absolute',
      ...
      {[0, 1, 2].map((i) => (<FlyCard key={i} i={i} frame={frame}/>))}
    </PvElement>);
};
