...
export const DeckDealFlyin: React.FC = () => {
    ...
    return (<PageCam2D src="textures/live/projects-empty.png" pageH={PAGE_H} keys={CAM_KEYS}>
      {/* dark brushed-metal table under the opening pile close-up */}
      {frame < METAL_FADE[1] ? (<PvElement as={"div"} style={{
      ...
      {grid.map((c, i) => {
            ...
            return (<PvElement as={"div"} key={`${c.file}-${i}`} style={{ transformStyle: 'preserve-3d' }}>
