...
export const SvgShapeMorph: React.FC = () => {
    ...
    return (<DesignStage bg="#0a0b10">
      <PvElement as={"svg"} viewBox="0 0 480 270" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', background: '#0a0b10' }}>
        <PvElement as={"g"} transform={`translate(${CX},${CY}) scale(${breath}) rotate(${rot}) translate(${-CX},${-CY})`}>
          <PvElement as={"path"} d={d} fill={`hsla(${hue},80%,58%,.14)`}/>
          <PvElement as={"path"} d={d} fill="none" strokeWidth={2} strokeLinejoin="round" stroke={`hsl(${hue},90%,66%)`} style={{ filter: `drop-shadow(0 0 8px hsla(${hue},90%,60%,.4))` }}/>
        </PvElement>
        <PvElement as={"text"} x={CX} y={252} textAnchor="middle" fill="#5b6480" opacity={0.4 + 0.6 * Math.abs(m - 0.5) * 2} style={{ font: '10px "SF Mono",Menlo,monospace', letterSpacing: '2px' }}>
