...
export const ClonerDepthEcho: React.FC = () => {
    ...
    return (<PvElement as={"div"} style={{ width: 1920, height: 1080, background: G.bg, overflow: 'hidden', position: 'relative' }}>
      ...
      <PvElement as={"div"} style={{ position: 'absolute', inset: 0, perspective: 1600, perspectiveOrigin: '58% 46%' }}>
        <PvElement as={"div"} style={{
          ...
          {/* 克隆队列：从后往前渲染保证遮挡正确 */}
          {Array.from(pvArrayCopy({ length: N }), (_, k) => N - k).map((idx) => {
            // idx 1..N，idx 越大越靠后
            const spread = spring({
            ...
            if (op <= 0.005)
                return null;
            return (<PvElement as={"div"} key={idx} style={{
                    position: 'absolute',
          ...
          {/* 本体 */}
          <PvElement as={"div"} style={{ position: 'absolute', transform: `translateZ(0px) scale(${heroScale.toFixed(4)})` }}>
