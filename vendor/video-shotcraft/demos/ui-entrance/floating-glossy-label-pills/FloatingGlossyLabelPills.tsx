...
const Skel: React.FC<{
    ...
    col: string;
    r?: number;
}> = ({ x, y, w, h, col, r }) => (<PvElement as={"div"} style={{
        position: 'absolute',
        left: x,
        top: y,
        width: w,
...
const Topbar: React.FC = () => (<PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, width: '100%', height: 16, background: '#fff', borderBottom: '1px solid #eceef1', zIndex: 1 }}>
    <Skel x={8} y={4} w={8} h={8} col={ACCENT} r={2}/>
    <Skel x={20} y={6} w={28} h={5} col="#b9bec6"/>
    <PvElement as={"div"} style={{ position: 'absolute', right: 8, top: 5.5, width: 5, height: 5, borderRadius: '50%', background: '#d8dbe0' }}/>
    <PvElement as={"div"} style={{ position: 'absolute', right: 18, top: 5.5, width: 5, height: 5, borderRadius: '50%', background: '#d8dbe0' }}/>
  </PvElement>);
// 左侧导航：一条高亮项 + 若干灰条
const Sidebar: React.FC = () => {
    const pvArrayCopy = useMotionCopy();
    return (<PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 16, width: 70, height: 'calc(100% - 16px)', background: '#f7f8f9', borderRight: '1px solid #eceef1' }}>
    <Skel x={8} y={8} w={36} h={8} col={ACCENT}/>
    {Array.from(pvArrayCopy({ length: 8 }), (_, i) => (<Skel key={i} x={8} y={26 + i * 11} w={30 + rand(i + 9) * 22} h={4} col={i === 1 ? ACCENT_LIGHT : '#d8dbe0'}/>))}
  </PvElement>);
};
...
const BCards: React.FC = () => {
    ...
    {Array.from(pvArrayCopy({ length: 3 }), (_, i) => (<PvElement as={"div"} key={i} style={{ position: 'absolute', left: 16 + i * 104, top: 64, width: 92, height: 170, border: `1px solid ${LINE}`, borderRadius: 6, background: '#fff' }}>
        <Skel x={10} y={12} w={38} h={6} col="#b9bec6"/>
        <Skel x={10} y={26} w={52} h={14} col={TITLE} r={4}/>
        {Array.from(pvArrayCopy({ length: 5 }), (_, k) => (<Skel key={k} x={10} y={52 + k * 13} w={44 + rand(i * 7 + k) * 26} h={4} col={FAINT}/>))}
...
const BDash: React.FC = () => {
    ...
    {Array.from(pvArrayCopy({ length: 4 }), (_, i) => (<PvElement as={"div"} key={i} style={{ position: 'absolute', left: 84 + i * 60, top: 44, width: 54, height: 32, border: `1px solid ${LINE}`, borderRadius: 6, background: '#fff' }}>
        <Skel x={7} y={7} w={34} h={8} col={TITLE}/>
        <Skel x={7} y={20} w={22} h={3} col={FAINT}/>
      </PvElement>))}
    <PvElement as={"div"} style={{ position: 'absolute', left: 84, top: 86, width: 232, height: 146, border: `1px solid ${LINE}`, borderRadius: 6, background: '#fff' }}>
      <Skel x={8} y={8} w={56} h={5} col={TEXT}/>
      <PvElement as={"svg"} viewBox="0 0 232 146" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        <PvElement as={"path"} d="M8 116 C 40 112, 56 62, 84 64 S 128 120, 152 116 S 196 50, 224 56 L224 134 L8 134 Z" fill={`rgba(${A_RGB},.18)`}/>
        <PvElement as={"path"} d="M8 116 C 40 112, 56 62, 84 64 S 128 120, 152 116 S 196 50, 224 56" fill="none" stroke={ACCENT} strokeWidth={2}/>
      </PvElement>
      {Array.from(pvArrayCopy({ length: 5 }), (_, i) => (<Skel key={i} x={12 + i * 44} y={138} w={22} h={3} col={FAINT}/>))}
    </PvElement>
  </>);
};
...
const BTable: React.FC = () => {
    ...
    {Array.from(pvArrayCopy({ length: 4 }), (_, i) => (<PvElement as={"div"} key={i} style={{ position: 'absolute', left: 84 + i * 60, top: 44, width: 54, height: 22, border: `1px solid ${LINE}`, borderRadius: 6, background: '#fff' }}>
        <Skel x={7} y={7} w={26 + rand(i + 3) * 14} h={8} col={TITLE}/>
      </PvElement>))}
    {Array.from(pvArrayCopy({ length: 7 }), (_, i) => (<PvElement as={"div"} key={i} style={{ position: 'absolute', left: 84, top: 78 + i * 22, width: 232, height: 18, borderBottom: '1px solid #f0f1f4' }}>
        <Skel x={0} y={6} w={70 + rand(i + 21) * 60} h={5} col={TEXT}/>
        {/* 状态色块保留功能色（异常淡红 / 正常淡绿） */}
        <Skel x={172} y={4} w={26} h={9} col={i % 2 ? '#f2d6d6' : '#cdeccf'} r={4}/>
        <PvElement as={"div"} style={{ position: 'absolute', left: 226, top: 6, width: 6, height: 6, borderRadius: '50%', background: '#28c06a' }}/>
      </PvElement>))}
  </>);
};
...
const BForm: React.FC = () => {
    ...
    {Array.from(pvArrayCopy({ length: 3 }), (_, i) => (<React.Fragment key={i}>
        <Skel x={84} y={56 + i * 26} w={34 + rand(i + 51) * 30} h={4} col={TEXT}/>
        <PvElement as={"div"} style={{ position: 'absolute', left: 84, top: 64 + i * 26, width: 120, height: 13, border: '1px solid #dcdfe4', borderRadius: 4, background: '#fff' }}/>
      </React.Fragment>))}
    <Skel x={84} y={140} w={120} h={15} col="#26282d" r={8}/>
    <Skel x={84} y={168} w={60} h={5} col="#b9bec6"/>
    {Array.from(pvArrayCopy({ length: 3 }), (_, i) => (<Skel key={i} x={84} y={180 + i * 9} w={110 + rand(i + 71) * 60} h={3} col={FAINT}/>))}
    <Skel x={218} y={44} w={40} h={5} col="#b9bec6"/>
    <Skel x={218} y={122} w={52} h={5} col="#b9bec6"/>
    {Array.from(pvArrayCopy({ length: 4 }), (_, i) => (<PvElement as={"div"} key={i} style={{ position: 'absolute', left: 218, top: 56 + i * 30 + (i > 1 ? 22 : 0), width: 100, height: 22 }}>
        <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 3, width: 13, height: 13, borderRadius: '50%', background: `hsl(${212 + i * 4},14%,${56 + i * 5}%)` }}/>
        <Skel x={19} y={3} w={48} h={4} col="#c2c6cc"/>
        <Skel x={19} y={11} w={32} h={4} col={FAINT}/>
        {/* 开关保留功能色（开=蓝 / 关=灰） */}
        <PvElement as={"div"} style={{ position: 'absolute', right: 0, top: 4, width: 18, height: 10, borderRadius: 6, background: i % 2 ? '#d6d9de' : '#2f7de1' }}>
          <PvElement as={"div"} style={{ position: 'absolute', top: 1.5, ...(i % 2 ? { left: 1.5 } : { right: 1.5 }), width: 7, height: 7, borderRadius: '50%', background: '#fff' }}/>
        </PvElement>
      </PvElement>))}
  </>);
};
...
export const FloatingGlossyLabelPills: React.FC = () => {
    ...
    return (<DesignStage bg="#fbfbfc">
      ...
      #fbfbfc`,
        ...
        {/* 漂移雾块 */}
        {FOGS.map(({ left, top, rgb }, i) => (<PvElement as={"div"} key={i} style={{
                position: 'absolute',
                left,
                top,
                width: 240,
