...
export const DocumentTypewriterReveal: React.FC = () => {
    ...
    let caretIdx = -1;
    blocks.forEach((_, i) => {
        if (frame >= cueFor(i) && frame <= cueFor(i) + WIPE + 2)
            caretIdx = i;
    });
    return (<>
      <PageCam2D src="textures/live/wbr-full.png" pageH={PAGE_H} keys={CAM_KEYS}>
        {blocks.map((p, i) => {
            ...
            const nameGrow = memberH2.has(i)
                ...
                : 0;
            return (<PvElement as={"div"} key={i}>
              {nameGrow > 0 ? (<PvElement as={"div"} style={{
              ...
              {coverT > 0 ? (<PvElement as={"div"} style={{
              ...
              {i === caretIdx ? (<PvElement as={"div"} style={{
        ...
        {PAST_WEEKS.map((w, i) => {
            ...
            return (<PvElement as={"div"} key={w.week} style={{
              ...
              <PvElement as={"div"} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <PvElement as={"span"} style={{ fontSize: 13, fontWeight: 500, color: '#1f2937' }}>{w.week}</PvElement>
                <PvElement as={"span"} style={{ fontSize: 10, color: '#9ca3af' }}>{w.date}</PvElement>
              </PvElement>
              <PvElement as={"div"} style={{ marginTop: 2, fontSize: 11, color: '#6b7280', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {w.title}
              </PvElement>
            </PvElement>);
        })}
