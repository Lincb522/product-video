    ...
    return (<DesignStage bg={BG}>
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ position: 'absolute', left: '50%', top: '50%', width: 440, height: 240, margin: '-120px 0 0 -220px' }}>
          ...
          <PvElement as={"svg"} width={440} height={240} viewBox="0 0 440 240" style={{ position: 'absolute', left: 0, top: 0, overflow: 'visible' }}>
            {SRCS.map((s, i) => {
            const { len } = GEOMS[i];
            const draw = seg(t, 0.04 + i * 0.045, 0.04 + i * 0.045 + 0.17, E.outQuad);
            const off = erase > 0 ? -erase * len : len * (1 - draw);
            return (<PvElement as={"path"} key={i} d={`M -22,${s.y} L 74,${s.y} C 186,${s.y} 214,${YC} ${XC},${YC}`} fill="none" stroke={TXT} strokeWidth={1.2} strokeDasharray={len} strokeDashoffset={off.toFixed(1)} opacity={(draw * (1 - clamp01((erase - 0.85) / 0.15))).toFixed(3)}/>);
        })}
          </PvElement>

          {/* 来源节点 + 数据包层 */}
          <PvElement as={"div"} style={{ position: 'absolute', left: 0, top: 0, width: 440, height: 240 }}>
            {SRCS.map((s, i) => {
          ...
          <PvElement as={"div"} style={{
            ...
            <PvElement as={"svg"} width={BADGE_SVG} height={BADGE_SVG} viewBox="0 0 24 24">
              <PvElement as={"path"} d="M12 0.8 L14.3 9.7 L23.2 12 L14.3 14.3 L12 23.2 L9.7 14.3 L0.8 12 L9.7 9.7 Z" fill={TXT}/>
            </PvElement>
          </PvElement>
