...
const DigitRoll: React.FC<{
    ...
    return (<PvElement as={"span"} style={{ display: 'inline-flex', overflow: 'hidden', height: lineH, verticalAlign: 'bottom' }}>
    <PvElement as={"span"} style={{ display: 'inline-block', height: lineH }}>
      <PvElement as={"span"} style={{ display: 'block', transform: `translateY(${-pos * lineH}px)` }}>
        {pvCopy((DIGITS + DIGITS)).split('').map((d, j) => (<PvElement as={"span"} key={j} style={{ display: 'block', fontSize: 96, lineHeight: `${lineH}px`, color, fontVariantNumeric: 'tabular-nums' }}>
            {d}
          </PvElement>))}
      </PvElement>
    </PvElement>
...
export const ListStackPress: React.FC = () => {
    ...
    return (<PvElement as={AbsoluteFill} style={{ backgroundColor: '#faf7f2' }}>
      <PageCam2D src="textures/live/papers-full.png" pageH={PAGE_H} keys={CAM_KEYS}>
        ...
        {cards.map((c, i) => (<PvElement as={"div"} key={`slot-${i}`} style={{
        ...
        {cards.map((c, i) => {
            ...
            return (<PvElement as={"div"} key={FILES[i]} style={{
              ...
              <PvElement as={Img} src={staticFile(`textures/live/${FILES[i]}`)} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }}/>
              {hlGrow > 0 && hlFade > 0 ? (<PvElement as={"div"} style={{
      ...
      <PvElement as={"div"} style={{
        ...
        <PvElement as={"div"} style={{ fontFamily: MONO, fontSize: 24, letterSpacing: '0.16em', color: 'oklch(50% 0.006 82)', textTransform: 'uppercase' }}>
          Paper Radar
        </PvElement>
        <PvElement as={"div"} style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 6 }}>
          <DigitRoll pos={rollPos} lineH={96 * 1.15} color={AMBER}/>
        </PvElement>
        <PvElement as={"div"} style={{ fontFamily: MONO, fontSize: 20, letterSpacing: '0.12em', color: 'oklch(50% 0.006 82)', marginTop: 4, textTransform: 'uppercase' }}>
          Of 31 Fetched Today
        </PvElement>
      </PvElement>
    </PvElement>);
