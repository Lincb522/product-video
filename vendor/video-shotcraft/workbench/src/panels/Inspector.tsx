...
import { MappingControl } from '../product-video/MappingControl';
import { TRANSITIONS } from '../product-video/Transition';
...
const PropControl: React.FC<{
  ...
  switch (field.type) {
    case "mapping": return <MappingControl field={field} value={value} onChange={onChange} onBegin={onBegin} />;
    case "text":
      return (
        <input
          type="text"
      ...
      <div className="inspector-scroll">
        {card && card.schema.length > 0 && (
          <section>
            ...
            {card.schema.map((field) => field.type === 'mapping' ? <div key={field.key} className="pv-mapping-group"><h3>{field.label}</h3><PropControl field={field} value={clip.props[field.key] ?? field.default} onChange={v => updateClipProps(clip.id, { [field.key]: v })} onBegin={begin} /></div> : (
        ...
        <section>
          ...
          {card?.kind !== 'audio' && <Row label="入场转场"><select value={clip.transition?.kind ?? 'cut'} onFocus={begin} onChange={e => updateClip(clip.id, { transition: { kind: e.target.value as typeof TRANSITIONS[number], duration: Math.min(clip.duration, clip.transition?.duration || Math.round(fps*.6)) } })}>{TRANSITIONS.map(kind => <option key={kind} value={kind}>{kind}</option>)}</select></Row>}
          {clip.transition && clip.transition.kind !== 'cut' && <Row label="转场时长"><input type="number" min={1/fps} max={Math.min(2, clip.duration/fps)} step={1/fps} value={clip.transition.duration/fps} onFocus={begin} onChange={e => updateClip(clip.id, { transition: { ...clip.transition!, duration: Math.min(clip.duration, Math.max(1, Math.round(Number(e.target.value)*fps))) } })} /></Row>}
          <Row label="起点">
            <span className="ctl-row">
              <input
                type="number"
