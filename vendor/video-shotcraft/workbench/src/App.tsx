...
import { NarrationPanel } from './product-video/NarrationPanel';
...
const ExportButton: React.FC = () => {
  ...
  return (
    <button
      ...
      title="将当前剪辑导出为 MP4，保存到工程 exports 目录"
      onClick={start}
    >
      导出成片
    </button>
...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (isEditable(e.target) || (e.target instanceof Element && e.target.closest('[role=dialog]'))) return;
  ...
  return (
    <div className="app">
      <header className="topbar">
        <span className="logo">Product Video <b>Studio</b></span>
        <input
          aria-label="工程名称" className="project-name"
          value={project.name}
